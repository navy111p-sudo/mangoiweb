...
//
// ── 창을 왜 «화·수·목 14:00–23:00» 으로 잡았나 ────────────────────────────
// 2026-09-02 사장님 지시(2차): 「수요일도 보류 창에 넣어줘.」
// 📜 같은 날 1차 지시는 「화요일과 목요일만 오후 2시부터 11시까지」였다. 그 뒤 배포 한 번이
//    수요일 17:35 KST 에 나갔고(run #2959), 그때 「수요일에도 카페24 미러 수업 40건이
//    있어 이 창이 보호하지 않는다」고 보고드리자 넣기로 정하셨다.
//    ⟹ 즉 **아래 «실측과 다르다» 던 부분이 수요일에 한해 실측 쪽으로 맞춰진 것**이다.
//
...
//
// 🔴 **요일은 아직 실측과 완전히 같지 않다 — 금·월이 남아 있다.**
//    2026-09-02 실측에서 앞으로 잡힌 수업은 목 45 · **수 40** · 화 30 · **금 7** · **월 5** 건이고,
//    토·일만 0건이다. 수요일 40건은 전부 `source='c24-mirror'` — 카페24 미러를 켜면서
//    카페24 수업이 망고아이 화상방으로 들어온 것이고, 미러 수업에 실제 접속이 남은 것도
//    확인했다(2026-09-01 `class-1046`). 그래서 이번에 수요일을 창에 넣었다.
//    ⟹ **금·월 수업(12건)은 여전히 이 «창» 이 보호하지 않는다.** 「다른 날은 수업이 없다」가
//       아니라 「그날은 창으로 막지 않는다」가 지금의 결정이다.
//    ⛔ 이 사실을 «수업이 없다» 로 바꿔 적지 말 것. 나중에 이 줄을 읽고 «그럼 안전하겠네» 로
//       넘어가면, 금요일에 수업이 끊긴 이유를 아무도 못 찾는다.
...
//       ⟹ 그래서 **오늘 실제로 막히는 것은 화·수·목 14:00~23:00 하나뿐**이다.
//       ⛔ 이것을 «금·월도 보호된다» 로 바꿔 적지 말 것. 지금은 사실이 아니다.
//
//    ⚠️ **그리고 그것을 깨우는 것은 «사장님이 정할 일» 이다.** 2026-09-02 지시의 취지는
//       「테스트 수업을 하는 시간대의 수업 질을 지킨다」였고, 요일은 그 지시가 정한다.
//       실접속 판정이 깨어나면 **금·월에도 배포가 막히기 시작해 그 지시를 벗어난다.**
...
//    좁히면서 배포 가능한 시간이 하루 11시간 40분 → 주 4일 전면 + 화·수·목도 15시간으로 늘었다.
//
...
export const CLASS_WINDOW_KST = Object.freeze({
    ...
    days: Object.freeze([2, 3, 4]),   // 화 · 수 · 목
    daysLabel: '화·수·목',
});
...
/** 지금이 수업 시간대인가.
 *  ⚠️ 요일부터 본다 — 화·수·목이 아니면 하루 종일 창 밖이다.
 ...
 *  🔴 창(화·수·목 14:00~23:00)만으로도 월·금 수업이 배포에 노출된다(2026-09-02 실측 12건).
...
export function decideHold({ now = new Date(), force = false, commitMessage = '', live = 0 } = {}) {
    ...
    /* 🔴 창 밖이어도 «지금 방에 사람이 있으면» 막는다.
       요일을 화·수·목으로 좁힌 대가(금·월이 창 밖)를 메우는 자리다(위 «실접속 판정» 블록). */
    if (Number(live) > 0) {
        return {
            hold: true,
            reason: 'live-class',
