...
//
// ── 창을 왜 «화·목 12:00–16:00» 으로 잡았나 ──────────────────────────────
// 2026-09-16 사장님 지시: 「스케줄이 바뀌어서 이제는 오후 12시부터 16시까지 막아줘.」
//    ⟹ 요일은 그대로 화·목이고 **시각만** 14:00~23:00 → 12:00~16:00 으로 옮겼다.
//    📜 그 전(2026-09-02)의 지시는 「배포가 안 나가는 건 화요일과 목요일만 오후 2시부터
//       11시까지. 이때만 우리가 테스트 수업을 해.」였다. 바뀐 것은 «테스트 수업을 언제
//       하는가» 이고, 「그때만 막는다」는 규칙 자체는 그대로다.
// 📜 같은 날 요일이 세 번 오갔다 — ① 화·목(1차) → ② 배포 한 번이 수 17:35 에 나가자 「수요일도
//    넣어줘」(#761, days [2,3,4]) → ③ 그날 저녁 「**수요일은 배포 보류 안 해도 돼 — 화요일과
//    목요일만 14시부터 23시까지**」(#761 revert, days [2,4]). 정본은 아래 CLASS_WINDOW_KST 두 줄뿐이고
//    deploy.yml 주석 · CLAUDE.md 함정표 · 하니스 두 개(class_window·class_guard)가 같은 말을 한다.
//    ⛔ 요일«이나 시각»을 다시 바꿀 때는 그 다섯 곳을 한 번에 — 한쪽만 고치면 «언제 보류되는지» 를 아무도 모른다.
//    ⚠️ 그리고 **몰아 배포 크론이 새 창 안으로 들어오지 않는지** 반드시 함께 볼 것(아래 ⛔).
//       2026-09-16 에 창을 12~16시로 옮기면서 «창이 열리기 전 마지막 기회» 였던 크론
//       하나가 창 안이 되어(자기잠금) **그 회차를 없앴다** — 옮길 자리가 없었기 때문이다.
//       ⛔ 「창이 닫힌 직후로 옮기면 된다」로 풀지 말 것. 16시 이후는 17·19·21시가 피크라
//          어디에 둬도 수업에 가깝고, 실측으로 16:16·16:27 접속이 있어 앞뒤 15분 규칙도 어긴다.
//          경위와 되살릴 때의 조건은 deploy.yml 의 schedule 주석에 적어 두었다.
//       ⛔ 그 시각을 여기 베껴 적지 말 것 — 정본은 deploy.yml 의 schedule 하나다.
//
// 🔴 **시간대는 «지난» 실측과 맞지 않는다. 그래도 12:00~16:00 으로 둔다 — 스케줄이 바뀌었다.**
//    [잰 것 — 2026-09-16, 최근 30일 attendance · `last_seen_at > 0` · `c24-*` 제외 = 실접속 769건]
//    KST 시간대별 실접속(화·목 / 월·수·금): 12시 14/3 · 13시 33/16 · 14시 33/26 · 15시 15/43 ·
//    16시 12/24 · 17시 28/39 · 19시 62/46 · 20시 45/15 · **21시 142/48** · 22시 13/4.
//    ⟹ 지난 30일만 보면 피크는 **21시**이고, 새 창(12~16시)이 막는 화·목 실접속은 95건으로
//       옛 창(14~23시)의 358건보다 **적다.** 즉 이 변경은 «지난 데이터 기준으로는» 보호를 줄인다.
//    ✅ 그런데도 이 값이 맞는 이유는 **사장님이 테스트 수업 시각을 옮기셨기 때문**이다.
//       위 숫자는 «옮기기 전» 을 잰 것이라, 앞으로의 수업을 예측하지 못한다.
//    ⛔ 그러니 이 표를 근거로 「21시가 피크니 창을 되돌리자」고 하지 말 것 — 되돌리려면
//       **지금 테스트 수업이 실제로 몇 시인지** 를 다시 재고 사장님께 확인해야 한다.
//    ✅ 다시 잴 때 쓸 한 줄(요일·시간대 분포):
//       SELECT CAST(strftime('%H', joined_at/1000,'unixepoch','+9 hours') AS INT) h, COUNT(*)
//       FROM attendance WHERE last_seen_at > 0 AND room_id NOT LIKE 'c24-%'
//         AND joined_at > (strftime('%s','now') - 30*86400)*1000 GROUP BY h ORDER BY h;
//
...
//       ⟹ 그래서 **오늘 실제로 막히는 것은 화·목 12:00~16:00 하나뿐**이다.
...
//    좁히면서 배포 가능한 시간이 하루 11시간 40분 → 주 5일 전면 + 화·목도 20시간으로 늘었다
//    (2026-09-16 에 창이 9시간 → 4시간이 되어 다시 늘었다).
//
...
/** 수업 시간대(KST). «끝» 은 포함하지 않는다 — 16:00 은 창 밖이다.
 ...
 *  🔴 창(화·목 12:00~16:00)만으로는 나머지 요일·시각이 배포에 노출된다.
 *     [잰 것 — 2026-09-16, 최근 30일 실접속 769건] 새 창이 막는 것은 **95건뿐**이고
 *     674건(88%)이 창 밖이다 — 월·수·금 전부(327건)와 화·목의 17시 이후(290건).
