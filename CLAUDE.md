...
| 머지했는데 그 변경만 배포가 안 나감 | **배포가 `cancelled` 로 끝날 수 있습니다** — `deploy.yml` 이 `cancel-in-progress: false` 인데도요. 짧은 간격으로 머지가 몰리면 발생합니다(2026-08-18 run#2467 실측). 보통은 **다음 배포가 그 내용을 함께 실어 나르므로** 문제가 없지만(`git merge-base --is-ancestor <내커밋> <배포된sha>` 로 확인), **그게 그날의 마지막 머지였다면 그 변경만 안 나갑니다.** 배포가 뜸한 시간대에 단독으로 머지했다면 결과를 한 번 보세요 |
