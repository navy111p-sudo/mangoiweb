...
- **이 저장소(test.mangoi.co.kr)는 아직 출시 전입니다.** 실사용자가 쓰는 곳은 **별도 시스템 `www.mangoi.co.kr`**(Apache/PHP)입니다.
- ⚠️ 다만 **DB `mango-db` 에는 Cafe24 에서 미러된 실제 학생 PII 가 들어 있고, 개발·운영이 같은 DB 를 씁니다.** 그래서 아래 **1-1 의 DB 경고는 그대로 유효**합니다 — 실험은 코드/화면에서만, DB 변경(DELETE/UPDATE/DROP)은 금지.
...
- 공동작업 안내(3인, 상세): [docs/망고아이_3인_공동개발_가이드_260811.pdf](docs/망고아이_3인_공동개발_가이드_260811.pdf)
- 팀 협업 가이드(쉬운 버전, 그림 포함): [docs/망고아이_팀_협업_가이드_260816.pdf](docs/망고아이_팀_협업_가이드_260816.pdf)
