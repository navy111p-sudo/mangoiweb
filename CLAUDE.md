...
| 앱(APK) 업데이트가 안 나감 | `mobile-app/**` 을 main 에 올리면 android.yml 이 APK 를 빌드해 `downloads/mangoi.apk`+`app-version.json` 을 main 에 커밋합니다. 예전엔 그 커밋이 `[skip ci]` 라 사이트 배포가 함께 안 나갔는데, **2026-08-14 android.yml 에서 그 문구를 제거해 이제 자동으로 배포까지 이어집니다.** 여전히 주의: 아무 커밋이든 **PR 제목·커밋 메시지에 skip ci 문구가 들어가면 머지 커밋이 배포를 건너뜁니다**(2026-08-14 실제로 밟음) |
