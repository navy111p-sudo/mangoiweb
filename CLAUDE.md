...
| 앱(APK) 업데이트가 안 나감 | `mobile-app/**` 을 main 에 올리면 android.yml 이 APK 를 빌드해 `downloads/mangoi.apk`+`app-version.json` 을 main 에 커밋하는데, 그 커밋 메시지가 **`[skip ci]` 라 사이트 배포가 자동으로 안 돕니다.** 다음 main 푸시(아무 PR 머지)까지 새 APK·매니페스트는 사이트에 안 나갑니다. APK 를 급히 배포하려면 머지 후 **추가로 main 에 한 번 더 푸시**(문서 수정 등)하거나 Actions 탭에서 deploy.yml 을 수동 실행하세요 |
