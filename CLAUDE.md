...
| 관리자 화면을 **영어로 렌더**했는데 한국어로 뜸 | `mangoi_lang='en'` + `mangoi_lang_by='user'` 만으로는 **안 바뀝니다.** `adm-lang-boot.js` 는 「사용자가 직접 고름」을 **그 계정에 한해서만** 인정해서, `mangoi_lang_uid` 가 **지금 세션 uid 와 같아야** 합니다(다르거나 없으면 «남의 선택»으로 보고 무시). 2026-08-19 안내서 영문판 캡처에서 실제로 밟음 — 영어로 찍은 줄 알았는데 전부 한국어였습니다 |
| 오른쪽 위 **사용자 메뉴**(마이페이지·비밀번호·로그아웃)가 안 열림 | **관리자 세션 키가 두 벌입니다.** 화면 대부분은 `mangoi_admin_session` 을 보는데, 그 모달(`adm-r21` ph115 `ph115OpenModal`)은 **`admin_session`** 을 봅니다. 한쪽만 심으면 이름은 뜨는데 눌러도 아무 일이 없습니다. 화면을 스크립트로 재현할 때는 **둘 다** 심으세요 |
| 자료실 안내서(「쉬운 사용법」)를 고쳐야 하는데 **그림밖에 없음** | 이제 소스가 있습니다 — **`tools/guide-shots/`**. 원고는 `deck/slides.{ko,en}.mjs`, 조립은 `deck/build.mjs`, 화면 캡처는 `shoot.mjs`(로컬 `public/` + 컨테이너 크로미움 + `/api/**` 스텁). 돌리는 법은 그 폴더 `README.md`. ⚠️ 원고와 `js/adm-s18.js` 의 `titles` 는 **장수·순서가 같아야** 합니다(캡션이 밀립니다). ⛔ 표에 «예시 학생·예시 금액» 을 채우지 마세요 — 안내서에 든 숫자는 사실로 읽힙니다 |
