...
| `index.html` 에 기능을 더했는데 하니스가 **첫 화면 무게**로 FAIL | `first_paint_budget_harness` 가 `index.html` 의 **blocking 합계**를 감시합니다. `/js/idx-main.js` 하나가 **849KB** 이고 이 파일은 **학생 29,000명 전원이 첫 화면에서 받습니다.** 여유가 12KB 뿐이라 5KB 만 더해도 넘칩니다(2026-08-19 귓속말 토스트에서 실제로 밟음). ✅ **강사·관리자만 쓰는 기능이면 별도 파일 + `defer`** 로 빼세요. `idx-main.js` 에는 «위임 5줄» 만 남깁니다. ⚠️ 그러면 **그 파일이 오기 전에 메시지가 올 수 있습니다** — 전역 큐(`window.__vcWhisperQ` 방식)에 담아 두고 로드되면서 비우세요. 큐가 없으면 «수업 중에 온 첫 메시지» 하나가 조용히 사라집니다. ⚠️ `--update` 로 기준선을 올리는 것은 **마지막 수단**입니다 — 그 예산은 필리핀 회선을 지키려고 둔 것입니다 | 📜 **2026-08-20 실측: 여유가 0.5KB 까지 줄었습니다** — 이제는 무엇을 더해도 넘칩니다. 참관 안전장치를 넣을 때 실제로 막혀서 `js/vc-observe-guard.js`(defer) 로 뺐습니다. ⚠️ 덤으로 **`?v=` 를 올리면 학생 29,000명이 850KB 를 다시 받습니다** — 주석 청소 같은 «공짜로 보이는» 수정도 그 값을 치릅니다.
