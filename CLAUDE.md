...
| 배포 커밋 = 배포 성공? | **아닙니다.** `deploy.ps1` 은 git commit·push(5/7)를 **Cloudflare 배포(6/7)보다 먼저** 합니다. `deploy:` 커밋이 있어도 wrangler 단계에서 실패했을 수 있습니다. 실제 반영은 라이브 `BUILD_STAMP` 로 확인하세요 |
| 하니스 요약 문구 | `run.mjs` 는 출력 끝부분의 `숫자+FAIL/실패` 를 실패 건수로 읽습니다. `PASS 36  FAIL 0` 이라고 쓰면 **36건 실패로 오인**됩니다. 실패 수를 먼저 쓰세요(`실패 0건 / 통과 36건`) |
| 난이도·가중치 설계 | 가중평균의 '가중치'로 난이도를 주면 **모든 문항 난이도가 같을 때 상쇄되어 무효**입니다. 점수 자체를 환산(상한)해야 변별이 생깁니다 |
