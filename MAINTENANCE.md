...
> 이제 어느 주소로 열어도 **같은 화상방(Durable Object)** 입니다.
>
...
>
> 📜 **이 배치는 세 번 사고를 냈습니다.** `wrangler.toml` 이 워커마다
...
>
> ⚠️ **8/19 에 「`-prod` 로 이전」이라고 적혔지만 실제로는 안 옮겨졌습니다.** 같은 작업기록에
> 「어느 워커에 붙어 있는지 **직접 확인하지 못했다**」가 나란히 있었는데 요약만 완료형이었습니다.
> **그래서 이번에는 사람이 `demo-1` 로 «서로 보이는 것» 을 확인한 뒤에 이 문단을 고쳤습니다.**
