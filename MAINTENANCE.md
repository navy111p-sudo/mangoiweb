...
> | `test.mangoi.co.kr` | **먼저 붙인 커스텀 도메인.** 살아 있음. 🔴 `mangoi.ai` 와 **다른 워커**(아래 배치 참조) | ❌ (죽이지도 말 것 — 아래 참조) |
...
>
> 🔴 **`test.mangoi.co.kr` 은 «기본» 워커에 붙어 있습니다 — `-prod` 가 아닙니다.**
> 이 저장소는 워커를 두 벌 배포하는데(`deploy.ps1` [6] 단계 · `deploy.yml` 1)2) 단계),
> `wrangler.toml` 이 워커마다 `new_sqlite_classes = ["VideoCallRoom"]` 을 따로 선언해
> **Durable Object 가 갈립니다.** 그래서 두 도메인은 **같은 방 번호를 넣어도 서로 다른 방**이고,
> 중국인 강선생님과 사장님이 8회 수업 동안 한 번도 만나지 못했습니다(2026-08-19 규명).
> D1·KV·R2 는 두 워커가 **같은 id 를 공유**하므로 **출석 기록만 보면 멀쩡해 보입니다.**
>
> ⚠️ **이 문단은 오래도록 「2026-08-19 에 `-prod` 로 이전」이라고 틀리게 적혀 있었습니다.**
> 그날 작업기록에도 「도메인이 어느 워커에 붙어 있는지 **직접 확인하지 못했다**」고 적혀 있습니다 —
> «옮기기로 했다» 가 «옮겼다» 로 적힌 것이고 **이전은 실제로 일어나지 않았습니다.**
> 그래서 2026-08-25 `class-895` 수업에서 **똑같은 사고가 재발**했습니다(둘 다 「참여자 1명」).
> ⏳ **아직 옮기지 않았습니다** — 절차·주의는 `docs/작업기록/260826_도메인_워커배치_문서정정.md`.
>
