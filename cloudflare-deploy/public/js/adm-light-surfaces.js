...
(function () {
  ...
  var SKIP_SEL = [
    ...
    // 🗓 화상수업 주간 스케줄 카드 — 유형별 색이 구분 정보다. 밝게 눌러 흰색으로 만들면
    //   무슨 수업인지 못 알아본다(사장님 지적, 2026-07-28).
    //   (2026-08-11) 색을 admin/weekly-schedule.html 과 같은 파스텔로 통일했다
    //   — 1:1 라벤더·그룹 소프트핑크·대체 스카이블루·레벨테스트 민트·휴무 회색사선.
    //   글자는 카드 자체 CSS(.ph54-ev)가 #1e293b 로 얹으므로 대비는 그쪽에서 확보된다.
    '.ph54-ev', '.ph54-ev *', '.ph54-slot', '.ph54-slot *', '.ph54-class', '.ph54-class *',
    // 👁 (2026-08-21) 참관·직접입장·위험 버튼 — 위 ph54 와 같은 이유로 «색이 구분 정보» 다.
    //   보라 = 참관(학생에게 안 보임) · 주황 = 직접 입장(학생에게 보임) · 빨강 = 되돌릴 수 없는 행위.
    //   실측(2026-08-21): 여기에 없던 동안 파랑 「연장」이 rgb(244,246,251) 로 눌려 흰 버튼이 됐다.
    ...
    /* 🙈 (2026-09-07) 관리자 교재 표의 «숨김» 토글 — 위와 같은 이유로 «색이 구분 정보» 다
       (초록 보임 · 빨강 숨김). 대비는 admin-inline-c.css 맨 끝 #textbooks-table 블록이 정한다.
       ℹ️ 실측(2026-09-07)상 «지금 팔레트» 는 밝은 바탕이라 이 페인터가 원래 안 건드린다 —
          즉 이 줄은 색을 어둡게 바꾸는 날을 위한 예약이고, 빼도 오늘은 화면이 안 바뀐다. */
    '.tb-hide-toggle', '.tb-hide-toggle *',
    ...
    /* 🟢🚪❓ (2026-09-01) 「강사 명부 (실데이터)」 상태 배지 — 위와 같은 이유로 «색이 구분 정보» 다
