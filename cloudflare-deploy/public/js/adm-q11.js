...
(function(){
  ...
  /* === 카드별 손자 메뉴 «데모» 매핑 ===
     ⚠️ (2026-08-18) 이 매핑은 **죽어 있다.** 이것을 쓰던 showFlyoutV2 는 2026-07-22 사장님 지시로
        첫 줄이 `return;` 이 되어(오른쪽 플라이아웃 중복 표시) 아무것도 그리지 않는다.
        사이드바에 실제로 보이는 손자 메뉴를 그리는 것은 **adm-r25.js(ph125)** 다.
        → 손자 메뉴 이름·목적지를 고치려면 여기가 아니라 adm-r25.js 를 고칠 것.
          여기만 고치면 화면은 하나도 안 바뀌는데 «고쳤다» 고 착각하게 된다.
     ⚠️ 그리고 이름 그대로 «데모» 다 — 카드에 그런 칸이 없어도 메뉴가 비어 보이지 않게 지어 넣은
        이름들이다. adm-r25.js 의 card-students-mgmt 는 2026-08-18 에 «진짜 하위칸» 으로 바꿨다. */
  var GRANDCHILDREN_MAP = {
    ...
    'card-alumni':              ['졸업생 등록','졸업생 목록','멘토 활동','동문 게시판','수신노트','수업 관찰','Ghost View'],
