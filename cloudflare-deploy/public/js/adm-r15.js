...
(function(){
  ...
  /* 🗺 (2026-08-15 사장님) 「메뉴 지도에 무엇이 들어 있는지 음성이 안 나온다」
     이 함수는 그때까지 «말하고 끝» 이었다. 그런데 메뉴 지도는 누르면 **다른 페이지로 옮겨 간다** —
     말을 시작해 놓고 페이지를 옮기면 그 순간 소리가 끊긴다(오디오 재생도, 브라우저 음성도).
     → «다 읽으면 알려 주는» 창구(onDone)를 열었고, adm-ia6 가 다 읽은 뒤에 페이지를 옮겼다.
     ...
     🐢 (2026-08-17 사장님) 「메뉴 지도 누르면 지도가 빨리 나오지 않아」 — 그 «다 읽고 나서 이동» 이
        원인이었다. 안내문이 95자라 12~14초짜리 소리가 되고, 그러면 아래 9초 안전장치가 먼저
        터져서 **누르면 9초 뒤에야** 이동이 시작됐다. 그래서 순서를 뒤집었다 — 지도는 곧바로 열고,
        안내는 도착한 지도 페이지가 읽는다(adm-ia6 의 openMenuMap 참고).
        ⚠️ 그 결과 지금 onDone 을 넘기는 곳은 없다. 이 창구와 9초 장치는 **그대로 둔다** —
           «읽고 나서 무언가 하기» 가 필요한 다음 자리를 위한 공용 API 다(window.admVoiceSay). */
  ...
  window.addEventListener('click', function(e){
    ...
    if (head){
      /* 🗺 「메뉴 지도」는 여기서 읽지 않는다. 누르면 곧바로 지도 페이지로 옮겨 가고,
         안내는 «도착한 지도 페이지» 가 읽는다(site-structure-map.html 의 「🔊 도착해서 읽어 준다」).
         여기서 읽으면 ① 이동하는 순간 반 마디에서 끊기고 ② 지도 쪽 안내와 두 번 말하게 된다.
         (2026-08-17 이전에는 adm-ia6 가 «다 읽고 나서 이동» 이라 이 줄의 이유가 «중복 방지» 하나였다.
          이유가 하나 늘었을 뿐 결론은 같다 — 지우지 말 것.) */
