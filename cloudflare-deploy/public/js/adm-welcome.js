...
(function () {
  ...
  function DIR() {
    var d = null;
    try { d = (window.MANGOI_GUIDE_DECKS || {})[L()]; } catch (e) {}
    return (d && d.dir) ? d.dir : (L() === 'en' ? '/guide/admin-easy-en/' : '/guide/admin-easy/');
  }
  // 슬라이드 번호는 /js/adm-guide-decks.js 에서 '제목'으로 찾아 쓴다.
  //   덱 구성이 바뀌어도 이 파일은 안 고쳐도 된다 — 그게 이 구조의 요점이다.
  //   n / en_n 은 공용 정의가 없을 때만 쓰는 폴백이다.
  function SNO(s) {
    var lang = L();
    var t = (lang === 'en') ? s.en_title : s.ko_title;
    try {
      if (window.mangoiGuideSlideNo && t) {
        var n = window.mangoiGuideSlideNo(lang, t);
        if (n) return n;
      }
    } catch (e) {}
    return (lang === 'en' && s.en_n) ? s.en_n : s.n;
  }

  // ── 슬라이드(핵심 사용법) — 실제 가이드 이미지 + 짧은 설명 ──
  var pad = function (n) { return (n < 10 ? '0' : '') + n; };
  // ⚠️ 두 덱은 '번역본'이 아니라 서로 다른 구성이다 — 한국어 18장 / 영어 24장이고 순서도 다르다.
  //    그래서 슬라이드를 '번호'로 가리키면 안 된다. 실제로 그렇게 했다가 어긋났다:
  //    캡션 "Left sidebar = every menu" 인데 그림은 영어 5번 "The screen, explained" 였다.
  //
  //    지금은 ko_title / en_title 로 가리키고, 실제 번호는 adm-guide-decks.js 에서 찾는다.
  //    → 덱 순서가 바뀌거나 슬라이드가 추가돼도 이 파일은 안 고쳐도 된다.
  //    ⚠️ 덱을 새로 만들면 adm-guide-decks.js 의 titles 만 맞추면 된다.
  //       단, 여기 적힌 제목과 철자가 정확히 같아야 찾는다(’ 같은 문장부호 포함).
  //    n / en_n 은 공용 정의가 안 실렸을 때만 쓰는 폴백이다.
  var SLIDES = [
    { n: 1,  en_n: 1,
