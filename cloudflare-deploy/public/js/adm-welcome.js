...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      //   요청: "지금보다 3분의 1 크기로 오른쪽 위에". 카드 폭 640px → 360px(가로 0.56배,
      //   차지하는 넓이는 약 1/3). 폰트·여백·버튼도 같은 비율로 줄여 비례를 유지한다.
      ...
      '#aw-card{position:relative;width:100%;max-width:360px;max-height:92vh;overflow:hidden auto;border-radius:14px;',
      '  background:linear-gradient(160deg,#38bdf8 0%,#2563eb 60%,#1d4ed8 100%);color:#fff;',
      '  box-shadow:0 18px 46px -14px rgba(0,0,0,.7);animation:awPop .28s cubic-bezier(.32,.72,0,1)}',
      '@keyframes awPop{from{transform:translateY(-12px) scale(.97);opacity:0}to{transform:none;opacity:1}}',
      '#aw-top{display:flex;align-items:center;justify-content:space-between;padding:9px 10px 2px}',
      '#aw-detail{display:inline-flex;align-items:center;gap:4px;cursor:pointer;border:none;',
      '  background:rgba(255,255,255,.22);color:#fff;font-weight:800;font-size:11px;padding:5px 9px;border-radius:99px}',
      '#aw-detail:hover{background:rgba(255,255,255,.32)}',
      '#aw-x{cursor:pointer;border:none;background:rgba(255,255,255,.22);color:#fff;width:24px;height:24px;',
      '  border-radius:50%;font-size:13px;font-weight:900;line-height:1;display:flex;align-items:center;justify-content:center}',
      '#aw-x:hover{background:rgba(255,255,255,.34)}',
      '#aw-hero{padding:3px 15px 2px}',
      '#aw-hello{font-size:18px;font-weight:900;letter-spacing:-.5px;line-height:1.15;margin:3px 0 2px}',
      '#aw-sub{font-size:11px;font-weight:600;color:#e6f0ff;opacity:.95;line-height:1.45}',
      '#aw-stage{position:relative;margin:8px 10px 0;border-radius:9px;overflow:hidden;background:#0b1220;',
      ...
      '.aw-nav{position:absolute;top:50%;transform:translateY(-50%);width:24px;height:24px;border-radius:50%;border:none;',
      '  cursor:pointer;background:rgba(255,255,255,.92);color:#0f172a;font-size:14px;font-weight:900;display:flex;',
      '  align-items:center;justify-content:center;box-shadow:0 3px 8px rgba(0,0,0,.35);z-index:2;line-height:1}',
      '.aw-nav:active{transform:translateY(-50%) scale(.9)}.aw-nav[disabled]{opacity:0;pointer-events:none}',
      '#aw-prev{left:6px}#aw-next{right:6px}',
      '#aw-caption{margin:7px 11px 0;background:rgba(255,255,255,.14);border-radius:8px;padding:8px 9px}',
      '#aw-ctitle{font-size:12px;font-weight:800;margin-bottom:3px}',
      '#aw-cdesc{font-size:10.5px;font-weight:600;line-height:1.55;color:#eaf2ff}',
      '#aw-dots{display:flex;gap:5px;justify-content:center;margin:8px 0 2px}',
      '.aw-dot{width:6px;height:6px;border-radius:99px;background:rgba(255,255,255,.4);cursor:pointer;transition:all .2s}',
      '.aw-dot.on{width:15px;background:#fff}',
      '#aw-foot{display:flex;align-items:center;gap:6px;flex-wrap:wrap;padding:8px 11px 12px}',
      '#aw-again{display:flex;align-items:center;gap:5px;font-size:10.5px;font-weight:700;color:#dbeafe;cursor:pointer;user-select:none;margin-right:auto}',
      '#aw-again input{width:13px;height:13px;accent-color:#fff;cursor:pointer}',
      '.aw-btn{cursor:pointer;border:none;font-weight:800;font-size:11px;padding:7px 12px;border-radius:8px;line-height:1}',
      '#aw-more{background:rgba(255,255,255,.2);color:#fff}#aw-more:hover{background:rgba(255,255,255,.3)}',
      '#aw-start{background:#fff;color:#1d4ed8;box-shadow:0 4px 10px -4px rgba(0,0,0,.5)}#aw-start:hover{background:#f1f5ff}',
      '@media(max-width:540px){#aw-overlay{padding:10px}#aw-hello{font-size:16px}#aw-hero{padding:3px 12px 2px}',
      '  #aw-foot{padding:8px 10px 11px}#aw-again{width:100%;margin:0 0 3px}.aw-btn{flex:1}}'
    ].join('');
