(function(){
  ...
  const RULES = [
    ...
    { kws:['ai 작문','ai작문','영작','영작문','작문 첨삭','글쓰기','ai write','writing'], action: () => location.href='/ai-write.html', label:'✍ AI 영작' },
    { kws:['영어 발음 코치','영어발음코치','영어 발음 연습','ai 음성코치','음성코치','발음 코치'], action: () => location.href='/speech-coach.html', label:'🗣 AI 음성코치' },
    { kws:['단어장','단어 장','내 단어장','어휘장','어휘','vocab','vocabulary','word list'], action: () => location.href='/vocab.html', label:'📖 단어장' },
    // 💡 판단력 훈련 — 다른 메뉴엔 있는데 AI 검색으로만 못 찾던 것을 추가(2026-08-15)
    { kws:['판단력','판단력 훈련','판단','상황 판단','judgment','judgement','decision'], action: () => location.href='/judgment.html', label:'💡 판단력 훈련' },
  ...
  function popularQuickList() {
    return [
      ...
      { label: '🗣 AI 음성코치', action: () => location.href = '/speech-coach.html' },
