/* ════════════════════════════════════════════════════════════════
   ...
   얼마나 거슬러 보나 — 방 이름이 «두 종류» 라 한 숫자로 못 정한다
     ...
     · mangoi-class (기본방·회의방) … 이름이 고정이라 앞 타임·어제 수업이 전부 같은 방 →
       길게 잡으면 남의 수업 대화가 넘어온다. 그래서 1시간만.
       📜 처음엔 3시간이었는데 2026-08-25 사장님 지시로 1시간으로 줄였다 — 3시간이면
       앞 타임 상담이 다음 학생 화면에 버튼 한 번으로 뜰 수 있어서(수업이 보통 25~50분).
       ⛔ 다시 늘리지 말 것. 「먼저 들어와 남긴 말」은 1시간이면 충분히 덮인다.
...
(function () {
  ...
  var WINDOW_SHARED = 1 * HOUR;   // mangoi-class 등 여러 수업이 돌려 쓰는 고정 방
  var MAX_ROWS = 200;
