...
(function(root){
  ...
  var isReview=false,finished=false,quizStartedAt=0;
  ...
  function showWordCard(word){
   var card=$('wordcard'),art=pictogram(String(word).toLowerCase()),hue=0;
  ...
  /* 🔠 2026-09-23 사장님 캡처 — 「English」 가 카드에 「english」 로 나왔습니다(낱말은 소문자로 저장).
     예문 «문장 한가운데» 에서 대문자로 쓰인 낱말(고유명사 — English·Monday·Korea)은 그 모양 그대로 보여 줍니다.
     ⚠️ 문장 첫머리의 대문자는 근거가 아닙니다(「School ends…」 의 School 은 보통명사) — 그때는 소문자 그대로.
     정답 비교는 normalize() 가 소문자로 맞추므로 영향이 없습니다. */
  function shownWord(item){var w=item&&item.word||'';if(!w)return w;var ex=example(item),rx=/[A-Za-z]+(?:'[A-Za-z]+)?/g,m;
   while((m=rx.exec(ex))){if(m[0].toLowerCase()!==w)continue;var before=ex.slice(0,m.index).replace(/["'“‘(\s]+$/,'');
    if(/[A-Z]/.test(m[0][0])&&before&&!/[.!?]$/.test(before))return m[0];}return w;}
  function answer(item){return mode==='words'?shownWord(item):(quiz&&difficulty()==='easy'?focusWord(item):scene(item).text);}
  ...
  function suspend(){stopMedia();if(current()){$('image').hidden=!$('image').getAttribute('src');if($('image').hidden&&mode==='words'&&picKind(current())==='card')showWordCard(shownWord(current()));}}
  ...
  /* ✍️ 교재 연동 쓰기 숙제(2026-09-24) — 「오늘의 A.i 학습」이 ?cur=1(&book=bts-05)로 곧바로 연다.
  ...
  /* 🧾 결과 기록 — 판 1행(/api/games/session)과 낱말별 정답·오답(/api/games/progress, game='scene-words').
     그 오답이 월간 성적표의 «약한 낱말» 이 된다. 게스트는 안 남긴다(성적표가 없다).
     ⚠️ 학습을 절대 방해하지 않는다 — 실패는 조용히 버린다(sendBeacon/keepalive). */
  function learnerId(){try{var u=localStorage.getItem('mangoi_uid');if(u)return u;var raw=JSON.parse(localStorage.getItem('mangoi_logged_user')||'null');if(raw&&(raw.uid||raw.user_id))return String(raw.uid||raw.user_id);}catch(e){}return '';}
  function post(url,obj){var body=JSON.stringify(obj);try{if(navigator.sendBeacon&&navigator.sendBeacon(url,new Blob([body],{type:'application/json'})))return;}catch(e){}try{fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:body,keepalive:true,credentials:'same-origin'}).catch(function(){});}catch(e){}}
  function recordResult(){if(isReview)return;var uid=learnerId();if(!uid||/^guest/i.test(uid))return;var right=0,wrong=0,events=[];
    items.forEach(function(it){var ok=review.indexOf(it)<0;if(ok)right++;else wrong++;if(mode==='words'&&it.word)events.push({item:String(it.word).toLowerCase(),ko:'',correct:ok});});
  ...
  function showImage(){if(!current())return;var s=scene(current()),img=$('image'),epoch=mediaId;clearTimeout(imageTimer);img.hidden=true;$('retry-image').hidden=true;
    ...
    if(mode==='words'&&picKind(current())==='card'&&current().word){img.removeAttribute('src');set('placeholder','');showWordCard(shownWord(current()));return;}
  ...
  function renderCard(){clearCard();var item=current();if(!item){set('count',tr('이 조건에 맞는 콘텐츠가 없어요. 다른 교재나 연습 종류를 골라 주세요.','No content matches. Choose another book or practice type.'));return;}
    $('card').hidden=false;passed=assisted=attempted=false;var s=scene(item);set('kind',mode==='words'?tr('WORD · 예문 속 단어','WORD · IN CONTEXT'):tr('SENTENCE · 상황 영상','SENTENCE · CONTEXT CLIP'));set('target',quiz?tr(mode==='words'?'빈칸의 단어를 써 보세요.':'영상을 보고 문장을 완성하세요.',mode==='words'?'Type the missing word.':'Complete the sentence after watching.'):(mode==='words'?shownWord(item):tr('장면을 보고 읽어 보세요.','Watch, then read the sentence.')));renderExample(item,false);
  ...
  $('series').addEventListener('change',function(){if(manifest){bookOptions();keepBook();loadBook();}});$('book').addEventListener('change',function(){keepBook();loadBook();});$('mode').addEventListener('change',resetItems);$('lesson').addEventListener('change',resetItems);$('level').addEventListener('change',resetItems);$('difficulty').addEventListener('change',resetItems);$('retry').addEventListener('click',function(){if(manifest)loadBook();else enter();});$('quiz').addEventListener('click',function(){startQuiz();});$('browse').addEventListener('click',resetItems);
  ...
  try{if(new URLSearchParams(location.search).get('cur')==='1')enter();}catch(e){}
