...
(function () {
  ...
  function goalRow(s, sample, en) {
    ...
    /* 목표가 «없는» 도구 — 개수만 말하고 «몇 개 남았다» 는 말하지 않는다.
       📜 2026-09-21 오전까지 games 가 여기였다(«판» 중앙값 40초라 목표를 두면 들락날락도
          «달성»). 같은 날 오후에 세는 단위를 items(문제)로 바꿔 games 도 목표를 가졌고,
          ⟹ 지금 표(TOOL_GOALS)의 도구 아홉은 전부 goal 이 있어 이 갈래로 오지 않는다.
       ⛔ 그래도 지우지 말 것 — **표에 없는 새 도구**가 그리로 떨어진다
          (today_goal_harness 가 그 갈래가 정본에 남아 있는지 짝으로 본다). */
    if (!g) {
      if (c <= 0) return '';
      return '<p class="goal-t muted">' +
