...
import { ensureNotifSchema, enqueueNotification } from './api-notify';  // 🔔 알림큐 (분리됨)  // 🧰 공용 헬퍼 (REFACTOR_PLAN 1단계 분리)
...
export async function handleMangoApi(
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    if (path.startsWith('/api/admin/teacher-profiles') || path.startsWith('/api/admin/teachers')
