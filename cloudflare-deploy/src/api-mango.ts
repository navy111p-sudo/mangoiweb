...
import { handleGamesApi } from './api-games';    // 🎮 게임/단어장·마이크로러닝 라우트 (분리됨)
...
응답 형식 (정확히 이 JSON 구조로만):
{
  ...
  "teacher_comment": "강사 종합 코멘트 (격려와 응원, 2~3 문장)"
    ...
    // ═══════════════════════════════════════════════════════════════
    // 📚 Phase VOC + 🧠 Phase ML → api-games.ts 로 분리 (REFACTOR_PLAN 1단계, 2026-07-14)
    //   /api/vocab/* 전체 + /api/admin/microlearn/* — null 반환 시 기존 라우팅 계속
    // ═══════════════════════════════════════════════════════════════
    if (path.startsWith('/api/vocab') || path.startsWith('/api/admin/microlearn/')) {
