...
${numbered}`;
    ...
    if (method === 'POST' && path === '/api/translate') {
      ...
      if (b.mode === 'note') {
        ...
        /* 🚨 (2026-08-10 라이브 실측) 모델이 한국어 문장 안에 **외국어 낱말을 섞어** 뱉는다.
              실제로 나온 것 — "서연이는 오늘 очень 잘했습니다"(러시아어),
                              "서연이는 오늘 rất 잘했습니다"(베트남어).
              한글이 있으니 «한국어» 검사는 통과해 버려서, 안 막으면 학부모에게 그대로 간다.
              🪤 처음엔 키릴·가나 같은 «문자체계 블랙리스트» 로 막았는데 베트남어는 라틴 문자라
                 그대로 새어 나갔다. → 블랙리스트가 아니라 **화이트리스트** 로 뒤집는다:
                 한글·ASCII·흔한 문장부호 «만» 허용하고, 그 밖의 글자가 하나라도 있으면 버린다.
                 (영어 낱말 th·AI 같은 것은 ASCII 라 그대로 통과한다) */
        const FOREIGN = /[^가-힣㄰-㆏ -~\s·…‘’“”—–「」『』₩°%]/;
