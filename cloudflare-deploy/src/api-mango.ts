...
import { ensureNotifSchema, enqueueNotification, sendPushToUser } from './api-notify';  // 🔔 알림큐·웹푸시 (분리됨)  // 🧰 공용 헬퍼 (REFACTOR_PLAN 1단계 분리)
...
export async function handleMangoApi(
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    if (path.startsWith('/api/admin/schedules') || path.startsWith('/api/admin/unassigned-students')
...
- "summary": a DETAILED 3-4 sentence overview that explains WHY this score, referencing the actual metrics (talk ratio, praise count, engagement) and their impact on the child's speaking practice. Make it genuinely helpful, not generic.
{
  ...
  "en": { "good": ["...", "..."], "improve": "...", "action": "...", "summary": "..." }
    ...
    // ═══════════════════════════════════════════════════════════════
    if (path.startsWith('/api/review-quiz/') || path.startsWith('/api/admin/review-quiz/')
