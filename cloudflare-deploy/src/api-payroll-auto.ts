...
export async function getPayrollAuto(env: any, year: number, month: number): Promise<any> {
  ...
  return {
    rows: list,
    summary: { teachers: list.length, total_completed, total_pay_php, updated_at },
  ...
  try {
    ...
    if (en) {
      const top = data.rows.slice(0, 10)
        ...
        .join(' / ');
      prompt =
        `You are the operations assistant of an online English academy. Summarize the ${year}-${String(month).padStart(2, '0')} teacher payroll below in clear English, 3-4 sentences, so the accounting manager can grasp it at a glance. Be factual and number-focused. Currency is Philippine Peso (₱).\n` +
        `Totals: ${data.summary.teachers} teachers, ${data.summary.total_completed} completed classes, total payout ₱${data.summary.total_pay_php.toLocaleString('en-US')}.\n` +
        `Top teachers: ${top}.\n` +
      ...
      prompt =
        ...
        `과장 없이 숫자 중심으로. 통화 단위는 페소(₱).\n` +
        `총 강사 ${data.summary.teachers}명, 완료수업 합계 ${data.summary.total_completed}회, 총 지급액 ₱${data.summary.total_pay_php.toLocaleString('en-US')}.\n` +
        `상위 강사: ${top}.\n` +
