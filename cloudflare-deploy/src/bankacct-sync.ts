...
const BANK_CAT_RULES: Array<[RegExp, string]> = [
  /* 🧑‍🏫 (2026-08-15 사장님 확인, 2026-08-19 «남궁국화» 정정) 은행 적요에 «급여» 글자가
       없어 이름으로 판별한다:
       · 메트로은행(필리핀) 송금 = 필리핀 강사 급여 → 손익계산서 II 강사급여 줄로
       · 이병엽·장지웅 = 직원 급여·정산 → 판관비 «직원급여» 줄로
       · «…환불» = 학생 환불 → 비용이 아니라 매출 차감으로
     ⛔ «남궁국화»(·「남궁국화A」)는 직원이 아니라 **지사수수료**다(2026-08-19 사장님 확인 —
     예전에 여기 잘못 들어가 있었음). 아래 급여 계열 규칙들에 안 걸리게 이 목록에서 빼고,
     지사수수료 판정은 accounting-reports.ts 의 「기타출금 쪼개기」 쪽에 맡긴다
     (franchises.owner_name 자동 매칭이 못 잡을 경우를 대비해 그쪽에 이름도 하드코딩해 뒀다).
     직원이 바뀌면 이 목록도 갱신할 것. */
