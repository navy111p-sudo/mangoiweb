...
/* ⚠️ 순서가 곧 우선순위다 — 먼저 걸리는 규칙이 이긴다.
   그래서 «좁은 규칙» 을 «넓은 규칙» 보다 위에 둔다. 실제로 이것 때문에 한 번 꼬였다:
     · 마케팅의 「네이버 광고」 는 통신의 「네이버」 보다 **위**에 있어야 한다.
       아니면 네이버 광고비가 통신비로 잡힌다.
     · 장비의 「구글 클라우드」 도 통신의 「구글」 보다 **위**여야 한다. */
const CAT_RULES: Array<[RegExp, string]> = [
  ...
  // 마케팅·장비를 통신보다 위로 — 아래 통신 규칙이 네이버/구글을 통째로 잡기 때문
  ...
  /* 📞 통신 — 통신사 + 온라인 구독 서비스. 원장님 회계 관행상 네이버·구글·AI 구독을
     통신비로 잡는다(2026-08-15 지시). 실측에서 「기타」에 있던 것들:
       ANTHROPIC* CLAUDE TEAM ₩107,085 · GOOGLE*GOOGLE DIGITAL ₩192,693 · 네이버 ₩36,300 */
  [/SKT|SK텔레콤|\bKT\b|LG\s*U|유플러스|텔레콤|통신|네이버|NAVER|구글|GOOGLE|ANTHROPIC|CLAUDE|OPENAI|CHATGPT|MS\s*365|카카오/i, '통신'],
