/* 📘 레벨테스트 «틀린 문제 다시 보기» — 정답·해설을 학생에게 돌려주는 판정 정본
   ...
   [🔒 정답이 새는 창을 좁힌다 — 여기가 이 파일의 핵심 계약이다]
     ...
     ⚠️ 그래도 «24문항을 전부 답하면 그중 틀린 것의 정답은 온다» 는 남는다 — 그것이
        이 기능 자체다. 막으려면 기능을 없애야 한다. 대신 «시험을 끝까지 봐야» 하는
        비용이 붙고, 배치 결과(students_erp.level)는 applyPlacementLevel 이 «이미 레벨이
        있으면 안 덮는다» 라 재응시로 수준을 올려도 학습도구 배정은 안 바뀐다.
