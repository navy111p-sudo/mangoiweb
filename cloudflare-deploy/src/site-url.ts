 ...
 *     ✅ (2026-09-01 이전 완료) 이제 세 도메인이 **같은 워커**(-prod)다.
 *     ⚠️ 여기 적혀 있던 「2026-08-27 이전 완료」는 사실이 아니었다 — 그때는 안 옮겨졌고
 *        2026-09-01 에 네 번째 사고(class-848)가 난 뒤에야 실제로 옮겨졌다.
 *     갈리면 화면·API 는 같은 코드라 차이가 없지만 **화상수업 방(DO)만 갈린다**(CLAUDE.md 0장·2장).
 *     ✅ 다시 갈리면 15분 감시견이 문자로 알린다 — `src/room-split-guard.ts` 가 이 SITE_HOSTS 를
 *        그대로 읽어 도메인별 DO 지문을 대조하므로, **이 목록에서 도메인을 빼면 그 도메인은 감시 밖이다.**
 ...
 * 새 링크를 만들 때는 쓰지 않는다 — 그건 SITE_ORIGIN 의 몫이다.
 * ✅ (2026-09-01) 이 주소도 이제 `-prod` 가 응답한다 — 이전 완료. 위 [주의] 참고.
