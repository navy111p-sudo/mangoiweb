 ...
 *   **매일 밤 덮였다.** 실측(2026-09-10): 그 표 29,485행의 번호 칸 네 개가 전부 0건이고, 8월에
 *   수업 리마인더 문자가 실제로 나갔던 체험계정 세 개(lt15·lt16·lt18)가 그렇게 번호를 잃었다.
 *   ✅ (2026-09-15) 그 «무조건 덮기» 자체는 cafe24-sync.ts 에서 막았다(COALESCE + DELETE 보존).
 *      ⚠️ 그래도 이 표가 정본인 것은 그대로다 — 카페24가 번호를 채우기 시작하면 그때부터는
 *         카페24 값이 이기고(정본이 그쪽), 그게 곧 «우리가 넣은 번호가 덮이는» 경우다.
 ...
 *
 * ⛔ `students_erp` 에 직접 쓰지 말 것 — **이유가 2026-09-15 에 바뀌었지만 금지는 그대로다.**
 *    (옛 이유) 카페24 동기화가 그 세 칸을 매일 밤 03:00 KST 에 **무조건** 덮었다
 *      — 실측 2026-09-10: 8월에 문자가 나갔던 체험계정 lt15·lt16·lt18 이 그렇게 번호를 잃었다.
 *    (지금 이유) 그 «무조건 덮기» 는 cafe24-sync.ts 에서 막았다. 그래도 ⓐ 카페24가 번호를
 *      채우기 시작하면 그 값이 이기고 ⓑ 읽는 정본(`phonesForStudent`)이 **이 표를 먼저** 보므로,
 *      명부에만 적으면 «화면과 문자가 서로 다른 번호를 쓰는» 상태가 된다.
 *    ⚠️ 다만 자가가입(api-students.ts)·레벨테스트 체험계정(leveltest-schedule.ts)·AI 운영비서
 *      (ai-command.ts) 세 경로는 이 표에 **안 쓴다** — 그 번호들에게는 cafe24-sync.ts 의
 *      보존이 유일한 보호막이다. 「override 가 있으니 저쪽은 빼도 된다」로 읽지 말 것.
 *
