...
export class VideoCallRoom {
  ...
  /* 🔴 (2026-09-15) ping 자동응답 근거를 «한 번도» 못 잡은 소켓에만 쓰는 넉넉한 상한.
     ...
     [원인 — 추론. 측정된 것이 아니다] 아래는 코드를 따라간 설명이고, 확정하려면
       이 파일이 새로 남기는 `auto=없음` 로그를 봐야 한다(⚠️ head_sampling_rate 가 0.05 라
       20건 중 19건은 로그 자체가 안 남는다). 고치기 전 로그에는 auto= 가 없었으므로
       그때 autoAt 이 0 이었는지는 «한 번도 측정된 적이 없다».
       위 설계는 getAutoResponseTimestamp 를 «생존 판단의 정본» 으로 삼는데,
