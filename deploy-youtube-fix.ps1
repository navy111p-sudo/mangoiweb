...
Write-Host ""
Write-Host "  [C] 로비 환영 음성"
Write-Host "    1) '수업입장' 클릭 시 환영 음성 자동 재생 (작은 볼륨 0.35)"
Write-Host "    2) 모달 좌상단 [🔊 음성 ON] 버튼 클릭 -> 음소거 (🔇 음성 OFF) + 정지"
Write-Host "    3) 음소거 상태 새로고침해도 유지 (localStorage)"
Write-Host "    4) 다른 화면 이동 / 탭 숨김 시 자동 정지"
