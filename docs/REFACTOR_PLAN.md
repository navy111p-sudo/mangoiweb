...
| | | (21차: 학생목록·graph-list·cafe24임포트 13매처 → api-admin, -60%. ⚠교훈: tsc는 전체 실행으로 확인(head 자르면 exit 오판·canViewPII 사후보강). 22차: Phase 9 메뉴6+레벨테스트+교재동영상 13경로(709줄) → api-admin, api-mango **5,463줄·-65%**. 다음: 마지막 대형 run(옛 5367-6483 → 현재 ~4100-5300, 50라우트: alerts·audit·briefing·dunning·exams·family·forecast·ghost·inquiry·nps·notifications 등) 순차 → 1단계 완료 임박). 23차: Phase GM 관리자통제 클러스터 31경로(933줄 — ghost·whisper·감사로그·브리핑·독촉·시험·가족·금칙어·예측·NPS·구독·동문회) → api-admin(5,943줄), api-mango **4,542줄·-71%**. 24차: WP 웹푸시→api-notify + I1~I2 신규상담→api-admin, api-mango **4,176줄·-73%**. 실측 잔여 32라우트(핸들러 기준): /api/ai/* 챗·영작 8, subscription 4, nps 2, textbook-files 2, ai-lesson-report 3, translate·i18n 2, class 2(입장경로=잔류), mango-videos·omnisearch·ai-command 등 산발. 25차: AW 영작+CF 친구챗 8매처 → 신규 **api-ai.ts**(582줄), api-mango **3,621줄·-77%**. 실측 잔여 ~24라우트(subscription 4·nps 2·textbook-files 2·ai-lesson-report 3·translate 2·class 2(잔류)·mango-videos·omnisearch·ai-command·best-recording 등 산발). 26~29차: NPS+구독·AI명령·AI학습리포트·교재파일+망고비디오 순차 이동. **api-mango 15,441→2,933줄(-81%)**. 잔여 실핸들러 9개=대부분 의도적 잔류(class 입장 2·translate/i18n 2·omnisearch·best-recording·eval ai-draft·get-lesson-video 등 부팅·공용·인터리브). ✅**1단계 백엔드 분리 목표 달성**(계획 목표 ~1,000줄 진입점화에 근접). **2단계 착수** 30차: admin.html 최대 인라인 블록(차트·학생관리·전역상태) 9,935줄 → /js/adm-core.js 추출(idx-*.js 전례·외부 classic script 전역공유). admin.html 43,853→33,917줄(-23%). 검증: node --check(추출본+잔여147블록)·</script>리터럴 부재·adm-core.js 실서버 200(153KB)·문법정상. ⚠한계: admin.html은 로그인 게이트(302)라 익명 curl 로 렌더 확인 불가 — **사장님이 관리자 로그인해 콘솔 1회 클릭검증 권장**(차트·학생목록 로드). 31차: 상위 인라인 7블록→adm-p1~7.js(admin.html 33,917→29,988줄). **🐛 라이브버그 발견·수정**: deploy.ps1 </body> 무조건치환이 JS문자열 깨뜨려 admin 3화면 장애중이던 것 복구+하드닝([[deploy-process]]). 검증=인라인 140블록 전수 node --check·adm-p 7개 실서버200. ⚠admin.html 게이트라 렌더는 사장님 클릭검증 필요. 32차: 인라인 12블록→adm-q1~12.js. admin.html 30,025→**26,459줄**(2단계 시작 43,853 대비 -40%). 하드닝 deploy.ps1 재발방지 실증(BUILD오염0). 검증=잔여128블록 전수 node --check·adm-q 12개 실서버200(신규파일 엣지전파 지연은 캐시버스터 재요청으로 해소). 33차: 인라인 30블록(120줄+)→adm-r1~30.js. admin.html 26,459→**21,678줄**(2단계 시작 43,853 대비 **-51%**, 절반돌파). 검증=잔여98블록 전수 node --check·adm-r 30개 실서버200. 추출 js 누적 50개(core+p7+q12+r30). 34차: 80줄+ 18블록→adm-s1~18.js. admin.html 21,678→**19,916줄(-55%)**. 추출 js 68개. **판단**: 잔여 80블록(~3,400줄)은 대부분 <80줄 소형 init/handler라 파편화(파일 68→100+개) 대비 실익 적음 → 대형블록 추출 목표는 달성. **권장 정지점**. 잔여 소블록은 필요 시 기능단위로만 묶어 추출. **⚠37차(유일한 실회귀·복구)**: 셀프진단 페이지가 PATCH /api/recordings/:id/status 404 검출 — 29차 이동분이 정규식(.test) 매처라 이물질검사·위임가드 양쪽 사각지대(path=== 만 스캔). 8모듈 정규식 25종 전수감사 → 미커버 2곳(recordings status·admin/textbook-files/:id) 가드 복구, 404→401 확인. **교훈: 검사·가드는 정규식 매처도 스캔**. ~~admin.html 렌더 클릭검증~~ → **✅완료(26-07-14 사장님 실검증)**: 관리자 콘솔 정상 — 학생목록 1000명 그래프DB 실데이터 렌더, 추출된 adm-p3/q12/r25~30/s12/s15/s16 등이 콘솔에 ph104~151 초기화 로그를 정상 출력(=68개 파일 실행 증명), 미로드 404·Uncaught 없음. ⛔2는 사이드바 상태로그 반복카운트로 판정. 35차(4단계 착수): package.json wrangler publish→deploy(제거된 명령) 수정+deploy:prod/typecheck 추가. 조사결과: 배포 실경로=deploy.bat/배포하기.bat→루트 deploy.ps1(정상), cloudflare-deploy/*.ps1 3개는 옛중복(실참조=worktrees 스냅샷뿐). **.claude/worktrees=908MB but git미추적·무시**(저장소 무관, 삭제는 git worktree라 파괴적→사장님 확인필요). 남은 4단계=옛 .ps1 이동(저가치)·worktrees 정리(확인필요). 36차(3단계): index.html VC·부팅 무관 8블록(2,900줄)→idx-x1~8.js — **9,358줄 VC+부팅 핵심블록은 불가침**(수업 안정성). index.html 31,203→**28,304줄**. 검증=8/8 문법·실서버200·참조8 + **브라우저 실렌더: 콘솔에러 0·홈 UI 정상**(공개페이지라 렌더검증 가능). admin.html 렌더만 사장님 클릭검증 잔존 | |
| | | | |
