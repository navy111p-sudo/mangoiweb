...
여성 내레이션 2종을 씁니다. 남성 녹음은 컷2가 옛 숫자(25분·200분)로 읽혀 있어
쓰지 않기로 했습니다 — 영상 파일은 지웠고, 녹음 원본과 스크립트는 `src/` 에 남아 있으니
필요해지면 컷2만 다시 녹음해서 되살릴 수 있습니다.

| 파일 | 사양 | 비고 |
|---|---|---|
| `mangoi_main_여성.mp4` | 1920×1080 / 30fps / **1:35.4** | 유튜브 본편 · 앞 1.2초 썸네일 |
| `mangoi_shorts_여성.mp4` | 1080×1920 / 30fps / **0:43.8** | 유튜브 쇼츠 · 앞 1.2초 썸네일 |

**제목**: 재미가 의지를 이깁니다 | 망고아이 A.I 화상영어
**썸네일**: `썸네일_본편_가로.png`(1280×720) · `썸네일_쇼츠_세로.png`(1080×1920)

> 두 영상 모두 **맨 앞에 썸네일이 1.2초 붙어** 있습니다(0.4초 디졸브로 본편 진입,
...
`src/내레이션_원본_여성.mp3` 가 현재 쓰는 녹음본입니다.
`src/내레이션_원본_남성.mp3` 도 남겨 뒀습니다 — 컷2만 다시 녹음해 `align3.py` 부터 돌리면
`timing_m` / `timing_vm` 으로 남성 버전을 다시 만들 수 있습니다.
