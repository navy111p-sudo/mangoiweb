...
# ── 큐 시트대로 구간 편성 ─────────────────────────────────────
mix = np.zeros(N)
mix += piano  * env_ramp(0.0, 10.0, 1.0)              * 0.42   # 00:00 정적
mix += swell  * env_ramp(0.0,  9.6, 0.8)              * 0.18
mix += drone  * (env_ramp(8.0, 25.0, 1.6) * 0.30 +             # 00:09 비어 있음
                 env_ramp(77.0, 95.0, 2.0) * 0.16)
mix += pad    * (env_ramp(23.0, 78.5, 1.5) * 0.30 +
                 env_ramp(77.5, 95.0, 2.0) * 0.40 +            # 01:18 패드만
                 env_ramp(93.5, 119.0, 1.5) * 0.30)
mix += bass   * (env_ramp(23.5, 78.0, 1.2) * 0.34 +            # 00:24 진입
                 env_ramp(93.5, 119.5, 1.2) * 0.34)
mix += arp    * (env_ramp(37.5, 78.0, 1.4) * 0.20 +            # 00:38 아르페지오
                 env_ramp(93.5, 119.5, 1.2) * 0.20)
mix += kick   * (env_ramp(23.5, 78.0, 1.0) * 0.30 +
                 env_ramp(59.5, 78.0, 0.8) * 0.24 +            # 01:00 최고 에너지
                 env_ramp(93.5, 119.5, 1.0) * 0.34)
mix += clap   * (env_ramp(59.5, 78.0, 0.8) * 0.30 +
                 env_ramp(107.5, 119.5, 0.8) * 0.26)
mix += hat    * (env_ramp(37.5, 78.0, 1.2) * 0.16 +
                 env_ramp(59.5, 78.0, 0.8) * 0.16 +
                 env_ramp(93.5, 119.5, 1.0) * 0.18)

# 전체 페이드
mix[:int(0.6 * SR)] *= np.linspace(0, 1, int(0.6 * SR))
tail = int(4.0 * SR)                                            # 01:56 여운
