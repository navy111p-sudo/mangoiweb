...
C = T.START + [T.TOTAL]
mix = np.zeros(N)
mix += piano  * env_ramp(0.0, C[1] + 0.6, 0.8)                  * 0.40
mix += swell  * env_ramp(0.0, C[1] + 0.2, 0.7)                  * 0.20
mix += pad    * (env_ramp(C[1] - 0.8, C[4] - 1.0, 1.0) * 0.30)
mix += bass   * (env_ramp(C[1] - 0.3, C[4] - 0.6, 0.8) * 0.34)
mix += arp    * (env_ramp(C[1] - 0.1, C[4] - 0.6, 0.9) * 0.20)
mix += kick   * (env_ramp(C[1] - 0.3, C[4] - 0.6, 0.8) * 0.30 +
                 env_ramp(C[2] - 0.2, C[3] - 0.1, 0.6) * 0.26)
mix += clap   * (env_ramp(C[2] - 0.2, C[3] - 0.1, 0.6) * 0.30 +
                 env_ramp(C[3] - 0.2, C[4] - 0.6, 0.6) * 0.24)
mix += hat    * (env_ramp(C[1] - 0.1, C[4] - 0.6, 0.9) * 0.16 +
                 env_ramp(C[2] - 0.2, C[3] - 0.1, 0.6) * 0.16)

# 전체 페이드
mix[:int(0.6 * SR)] *= np.linspace(0, 1, int(0.6 * SR))
tail = int(3.0 * SR)                                            # 마지막 여운
