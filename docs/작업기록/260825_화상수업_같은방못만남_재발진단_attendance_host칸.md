# 화상수업 「같은 방인데 서로 못 만남」 재발 진단 — Hannah/delaware class-895-20260825
...
1. 학생 `delaware`(김연숙), 강사 `mangoi_167`(Hannah)
...
> 이름 정정: 제보 원문은 강사를 「Farrah」로 적었으나 사장님이 이후 「Hannah」로 정정.
> 참고로 CLAUDE.md 2장 「계정이 두 개인가」 항목에 `mangoi_167` = HANNAH 라는 사실이
> 이미 2026-08-24(하루 전) 실측으로 적혀 있다(`Mangoi_167`(대문자 M, 8/24 생성·연결 안 됨)와
> 나란히 있었고, 그 대소문자 계정 이중화 자체는 같은 날 이미 고쳤다고 기록됨). 그 항목의
> 증상(「강사 화면이 수업 없음」)은 오늘 증상(「같은 방인데 서로 못 만남」)과 다르므로
> **직접적인 원인으로 보지는 않았지만**, 로그인 계정이 최근 이 계정 자체에서 문제가 났던
> 이력이 있다는 점은 참고할 가치가 있어 남겨 둔다.
...
1. 🔴 **사람이 Cloudflare 대시보드에서 확인해야 하는 것**: `webrtc-unified-platform` 과
   ...
   남아 있을 가능성을 배제할 수 없다.)
2. 🔴 **Hannah(`mangoi_167`) 본인 확인 필요**: 화면 1(데스크톱, 창 제목 「Mangoi 화상솔루션」)이
