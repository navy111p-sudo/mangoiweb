...
| | |
|---|---|
| **증상** | 「무거우면 자꾸 튕겨나가」 — 강사 재입장 61.9% |
| **관측** | `X-Turn-Source: public-fallback` = 모든 수업이 무료 공개 TURN 으로 릴레이 중 |
| **제가 내린 결론** | 🔴 「TURN 시크릿이 없다」 → **틀렸습니다** |
| **실제** | 시크릿은 **워커 두 벌 모두** 있었습니다(대시보드 실측). 시크릿이 있는데도 **24시간 넘게 발급이 한 번도 성공하지 못한 구간**이 있었고, 그것이 그날 아침에 끝났습니다 |
| **왜 틀렸나** | `public-fallback` 하나가 ①키 없음 ②CF 거절 ③연결 실패 **셋을 뭉뚱그려서** |
| **고친 것** | ⓐ `X-Turn-Detail` 로 셋을 가름 ⓑ 배포·감시견이 detail 마다 다른 «할 일» 을 안내 ⓒ 영상이 죽은 사람도 품질로그에 남게 |
| **아직 모르는 것** | ⏳ 왜 24시간 넘게 실패했나 (Workers 로그에 답이 있을 수 있음 — 7-2) |

> ⚠️ **아래 2장에 제가 «시크릿이 없다» 고 단정한 문장이 그대로 남아 있습니다.**
> 지우지 않았습니다 — 어떻게 틀렸는지가 이 기록의 핵심이기 때문입니다.
> 정정은 **6-2 장**에 있습니다.
...
> 🔴 **이 마지막 한 줄이 틀렸습니다.** 앞의 두 줄(「24시간 안에 성공 0회」)까지는 맞고,
> 거기서 «그러니 키가 없다» 로 건너뛴 것이 오진입니다 — 키가 있어도 CF 가 거절하면
> 똑같은 모습이 됩니다. 시크릿은 **워커 두 벌 모두 있었습니다.** 정정은 **6-2 장**.
...
| ~~**`src/index.ts` 를 손대기**~~ | 처음엔 「공동 금지구역이고 애초에 코드 문제가 아니라 시크릿 문제」라고 적었습니다. **두 전제가 다 틀렸습니다** — 시크릿 문제가 아니었고, 사장님이 직접 지시하셔서 `handleTurnConfig` 안에서만 만졌습니다(7-3) |
| **CF 응답 «본문»(`cfResp.text()`)을 detail 에 싣기** | 그게 제일 자세하지만 이 API 는 `Access-Control-Allow-Origin: *` 라 **로그인 없이 누구나** 부릅니다. 본문에는 계정·키 관련 문구가 들어올 수 있어 상태 «코드» 까지만 싣습니다(하니스 3부가 «새지 않는가» 를 검사) |
| **detail 값을 읽는 쪽에서 «모르면 나쁨» 으로 판정** | 값은 앞으로 늘어납니다. 모르는 값을 나쁨으로 치면 새 값이 생길 때마다 거짓 경보가 납니다 → `deploy.yml`·감시견 둘 다 **모르면 표시만** 합니다 |
| **`public-fallback` 라벨을 그대로 두기** | detail 을 실어 놓고도 라벨이 「시크릿 미설정」이라고 단정하면 읽는 사람은 라벨만 보고 또 키를 넣으러 갑니다. 배포 로그를 눈으로 읽다 발견했습니다(6-4) |
...
**PR #522·#528 에서 추가로 바뀐 것** (오진 정정 + `X-Turn-Detail`)

| 파일 | 무엇 |
|---|---|
| `cloudflare-deploy/src/index.ts` | `handleTurnConfig` 에 `X-Turn-Detail` — `cache`·`ok`·`no-secrets`·`cf-http-<코드>`·`cf-fetch-error`. 두 `console` 로그에도 같은 값. ⚠️ **공동 금지구역** — 사장님 직접 지시로 만졌고 라우팅·인증 게이트·DO·자산 처리는 한 줄도 안 건드렸습니다 |
| `.github/workflows/deploy.yml` | 요약 표에 detail 칸 + detail 마다 다른 «할 일». ⛔ 「`no-secrets` 가 아니면 키를 새로 넣지 마세요」를 못 박음 |
| `ops/mangoi-watchdog.sh` | 로그·문자에 detail 반영. 403 일 때 「키는 이미 등록돼 있으니 «없다» 가 아닙니다」 |
| `test-harness/turn_detail_harness.mjs` | **신규 30종** — `handleTurnConfig` 를 소스에서 오려 내 `node:vm` 으로 **실제로 돌립니다** |
| `test-harness/vc_turn_quality_harness.mjs` | 검사 하나가 «앞 4000자» 라는 길이에 기대고 있어 단계가 길어지자 깨진 것을 고침(길이 → 단계의 끝, 앵커도 주석 → `- name:` 헤더) |
...
**PR #522·#528 (`X-Turn-Detail`) 검증**

* 타입체크 exit 0 · `run.mjs --fast` **PASS 217 / FAIL 0** · `ci-gates.sh` 4종 통과
* `turn_detail_harness` **30종** — 네 갈래(캐시·성공·LKG·무료폴백)를 실제로 돌려
  갈래마다 어떤 detail 이 나오는지 읽습니다. 403·429·연결실패까지 각각.
* **되돌려서 확인**: `detail = 'cf-http-…'` 두 줄을 빼면 넷이 전부 `no-secrets` 로 뭉쳐지며
  **4건 FAIL** — 즉 이 하니스는 «사고 당시 상태» 를 그대로 재현해 잡습니다.
* 배포 라벨 검사도 되돌려 확인(옛 문장 복원 → 1건 FAIL).

---

## 6-2. 🔴 배포 후 관측 — 그리고 **제 결론이 틀렸다는 것이 드러났습니다**
...
세 번에 나눠 나갔습니다. 두 번째·세 번째가 **제 오진을 되돌린 것**입니다.

| PR | 무엇 | CI | 배포 |
|---|---|---|---|
| **#518** | TURN 경로 감시(배포·감시견) + 품질로그 사각지대(`novideo`) | ✅ 91초 | ✅ run #2717 · 07:38:29~07:40:58 (2분 28초) |
| **#522** | 「시크릿이 없다」 오진 정정 + `X-Turn-Detail` 신설 | ✅ 97초 | ✅ run #2725 · 08:18:44~08:21:13 (2분 28초) |
| **#528** | 배포 라벨에 남아 있던 「시크릿 미설정」 단정 제거 | ✅ 93초 | ✅ run #2731 · 10:03:32~10:05:59 (2분 27초) |

**배포된 TURN 상태 실측** (러너에서 두 워커를 직접 호출)

| 시각(UTC) | 기본 | production |
|---|---|---|
| 07:40:54 | `kv-cache` | `kv-cache` |
| 08:21:03 | `kv-cache (cache)` | `kv-cache (cache)` |
| 10:05:55 | `kv-cache (cache)` | `kv-cache (cache)` |

캐시 수명이 1시간이므로 **09:05 이후에도 CF 발급이 성공**했습니다 — 아침 이후로 계속 정상입니다.

⚠️ **`ok`(방금 새로 발급)는 좀처럼 안 보일 것입니다** — 수업 입장마다 이 API 를 부르므로
캐시가 늘 따뜻합니다. `cache` 가 곧 «최근 1시간 안에 성공했다» 이므로 그것으로 충분합니다.

⚠️ 저는 배포된 화면·API 를 **직접 못 봅니다**(프록시가 `mangoi.ai`·`workers.dev` 를 403 으로
막습니다). 위 값은 전부 GitHub 러너에서 잰 것이고, 이 환경에서 얻을 수 있는 유일한 실측입니다.
