...
/* 🗺 「사이트 구조도」 항목의 역사 —
     2026-08-15 옛 사이드바 「시스템」 그룹에만 있었는데 ia6 가 그 그룹을 통째로 감춰서
       **아무도 볼 수 없었다**. 그래서 새 사이드바에도 자리를 줬다.
     2026-08-16 사장님: «어차피 메뉴판 맨 위 「메뉴 지도」와 같은 거야» — 실제로 같은 페이지로
       갔다. 같은 곳으로 가는 문이 둘이면 «둘이 다른 건가?» 를 매번 생각하게 만든다. 뺐다.
   그래서 이 검사가 지키는 것이 뒤집힌다 — 「있어야 한다」 → 「중복이면 안 된다」.
   다만 **원래 지키려던 것은 그대로다**: 지도로 가는 길이 화면에 하나는 있어야 한다.
   그 길이 맨 위 「메뉴 지도」이고, 그건 아래 ⑪ 에서 따로 검사한다. */
{
  const start = ia6.indexOf('var GROUPS = [');
  const end = ia6.indexOf('\n  ];', start);
  const body = start >= 0 && end > start ? ia6.slice(start, end) : '';
  check('시스템 그룹에 「사이트 구조도」 항목이 없다 (맨 위 「메뉴 지도」와 중복 — 되살리지 말 것)',
    body.length > 100 && !/ko:\s*'사이트 구조도'/.test(body));
}
check('지도 페이지는 그대로 있다 (「메뉴 지도」가 여는 곳)',
  existsSync(resolve(__dir, '../cloudflare-deploy/public/admin/site-structure-map.html')));
check('옛 목차 파일도 남아 있다 (지도로 넘기는 문 — 옛 링크·즐겨찾기가 404 가 되면 안 된다)',
  existsSync(resolve(__dir, '../cloudflare-deploy/public/admin/site-structure.html')));
check('옛 사이드바에도 중복 항목이 없다 (ia6 가 못 뜨는 날 되살아나면 안 된다)',
  !/location\.href='\/admin\/site-structure\.html'/.test(html));
