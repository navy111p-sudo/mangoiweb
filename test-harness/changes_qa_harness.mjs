...
// ⚠️ 예전엔 `git show … | wc -l` 이었는데 wc 는 유닉스 도구라 PowerShell 로 돌리면 항상 실패(-1)해서
//    integrity 검사가 환경에 따라 거짓 실패했다(배포 게이트가 멀쩡한 코드를 막음). 줄 세기는 Node 로 한다.
const headLines = rel => { try { return execSync(`git -C "${ROOT}" show HEAD:${rel}`, { maxBuffer: 1<<28 }).toString().split('\n').length - 1; } catch { return -1; } };
...
// find/wc 대신 Node 재귀 탐색 — 셸 종류와 무관하게 같은 결과가 나오게(위 headLines 와 같은 이유)
ok('cleanup:no .bak served under public/', (()=>{
  const walk = (dir) => { let hit = false;
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      if (e.isDirectory()) { if (walk(join(dir, e.name))) hit = true; }
      else if (/\.bak/i.test(e.name)) hit = true;
    }
    return hit; };
  try { return !walk(CD + '/public'); } catch { return false; }
})());
