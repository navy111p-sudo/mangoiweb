...
console.log('\n⑤ 배선 — sessions/today 와 지정 API 에 실제로 붙어 있는가');
{
  ...
  if (iPut > 0) {
    ...
    check('⑤-7 ⛔ 지사·대리점을 막는다', /isOrgScopedRole\s*\(/.test(body) && /forbidden_scope/.test(body));
    check('⑤-8 ⛔ 강사는 «자기 수업» 인지 확인한다', /teacherOwnsSchedule\s*\(/.test(body) && /forbidden_not_my_class/.test(body));
...
console.log('\n⑥ 화면 — 지정 버튼과 학생 안내가 실제로 붙어 있는가');
{
  ...
  check('⑥-5 링크 주소를 화면이 조립하지 않는다 (서버가 준 d.link 를 쓴다)',
    /d\.link/.test(tBare) && !/openRoomOverride[\s\S]{0,2600}location\.origin/.test(tBare));
  check('⑥-6 ⛔ window.open 으로 링크를 열지 않는다 (인앱 브라우저는 null 만 준다)',
    !/openRoomOverride[\s\S]{0,2600}window\.open\s*\(/.test(tBare));
  ...
  check('⑥-10 «사람이 안 쳤을 때만» 안내한다 (스스로 친 사람에게는 거짓말이 된다)',
    /!typedAtJoin/.test(dBare) && /typedAtJoin\s*=/.test(dBare));
  check('⑥-11 회의방일 때만 안내한다', /\/\^meet-\/i\.test\(room\)/.test(dBare));
