...
//     ① 「네이버 광고」가 통신비로 새지 않는다 (마케팅이 통신보다 위)
//     ② 「구글 클라우드」가 통신비로 새지 않는다 (장비가 통신보다 위)
...
eq('네이버(구독)', cat('네이버', ''), '통신');
eq('구글 클라우드', cat('구글 클라우드', ''), '장비');
eq('GOOGLE*GOOGLE DIGITAL', cat('GOOGLE*GOOGLE DIGITAL', ''), '통신');
eq('카카오모먼트(광고)', cat('카카오모먼트', ''), '마케팅');

/* ── ② 원장님 지시(2026-08-15) — 온라인 구독은 통신비 ─────────────────── */
console.log('  ── 온라인 구독 → 통신');
eq('ANTHROPIC* CLAUDE TEAM', cat('ANTHROPIC* CLAUDE TEAM', ''), '통신');
