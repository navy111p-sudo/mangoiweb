...
/* ── ③ 사람이 확정 «전에» 근거를 본다 ──────────────────────────────────────
   🔴 이 표에는 수강료를 «치는 칸이 없다»(hidden). 그래서 정상 등록에서는 기준가가 늘 비어
      대리점 단가 갈래가 **매번** 걸리고, 예전에 「월 수강료가 0원입니다」 로 사람을 세우던
      자리를 그냥 통과해 구독 금액으로 들어간다. 안 보여 주면 «아무도 치지 않은 금액이
      조용히 청구되는» 상태다 — 그래서 «배수가 1배여도 항상» 보여 줘야 한다. */
check('③ 확정 계획이 기준가·배수를 함께 내려준다', /base_fee_krw:/.test(activate) && /length_multiplier:/.test(activate));
check('③ 확정 계획이 «자동으로 세운 값인지» 를 추측하지 않고 저장된 값으로 알려준다',
  /fee_source:/.test(activate) && /_addEnrCol2\('fee_source',\s*'TEXT'\)/.test(apiAdmin));
check('③ 확정 패널이 요금 근거를 그린다', /요금 근거/.test(core));
check('③ 근거를 «배수가 1배일 때도» 보여 준다 (자동 금액은 대개 20분 = 1배다)',
  !/length_multiplier\)\s*!==\s*1[\s\S]{0,80}요금 근거/.test(core) &&
  !/요금 근거[\s\S]{0,400}length_multiplier\)\s*!==\s*1/.test(core),
  '1배일 때 숨기면 자동 금액이 화면에 한 번도 안 나온다');
check('③ 자동으로 세운 금액은 «자동» 이라고 말한다',
  /agency_price/.test(core) && /자동/.test(core));

/* ── ③-2 등록 «직후» 나가는 카톡·CSV·워드가 최종 금액을 쓰는가 ──────────────
   🔴 화면이 폼 값(곱하기 «전» 기준가)을 그대로 적으면, 40분 신청이 DB·구독에는 20만원인데
      문서에는 10만원으로 나간다. 서버가 응답에 실어 준 계산 결과를 받아 적어야 한다.
      ⛔ 화면이 스스로 곱해서 맞추면 «두 번 곱하기» 다. */
check('③-2 서버가 등록 응답에 계산 결과를 실어 준다', /fee:\s*_fee/.test(apiAdmin));
check('③-2 단건 내보내기가 서버가 준 최종 금액을 쓴다',
  /d\.fee\s*&&\s*d\.fee\.monthlyFeeKrw != null/.test(core));
check('③-2 일괄 내보내기도 서버가 준 최종 금액을 쓴다',
  /j\.fee\s*&&\s*j\.fee\.monthlyFeeKrw != null/.test(core));
