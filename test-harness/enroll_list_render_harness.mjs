...
for (const lang of ['ko', 'en']) {
  ...
  /* ✅ (2026-08-12) 등록 = 확정으로 합치면서 버튼 구성이 바뀌었다.
     ...
     ✅ (2026-08-20) 「▶ 수강시작」도 없앴다 — enroll-activate.ts 의 확정 파이프라인이
     class_schedules 를 이미 status='active' 로 만들어서, 그 버튼은 enrollments.status 만
     confirmed → active 로 바꿀 뿐 실제로는 아무 일도 안 났다. */
  const goneStart   = lang === 'en' ? '▶ Start'   : '▶ 수강시작';
  const wantCancel  = lang === 'en' ? '✕ Cancel'  : '✕ 취소';
  const goneConfirm = lang === 'en' ? '✓ Confirm' : '✓ 확정';
  ok(!html.includes(goneStart) && html.includes(wantCancel),
     `⛔ 「${goneStart}」 버튼은 없어야 한다 / 「${wantCancel}」 은 있어야 한다`);
  ...
  // 2) 수강시작 버튼이 없으므로 active 행에 남는 버튼은 «취소» 하나뿐 — 활성 건이라 잠기지 않는다
  ...
  ok(btns4.length === 1 && btns4[0].target === 'cancelled' && !btns4[0].disabled,
     'active 행 버튼 1개 (취소, 잠기지 않음) — 실제 ' + JSON.stringify(btns4.map(b => b.target + (b.disabled ? '(잠김)' : ''))));
