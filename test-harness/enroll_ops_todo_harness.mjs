...
const stripTs = (s) => s.replace(/\bas any\[\]/g, '').replace(/\bas any\b/g, '').replace(/:\s*any\b/g, '');
...
ok('날짜 순서대로', days.join(',') === '2026-10-03,2026-10-20');

let r2 = null;
try { r2 = fn ? await fn({ DB: { prepare() { throw new Error('D1 down'); } } }, '2026-09-25') : null; } catch (e) { r2 = { threw: true }; }
ok('조회가 죽으면 던지지 않고 ok:false (0건으로 위장하지 않음)', r2 && r2.ok === false && !r2.threw);
