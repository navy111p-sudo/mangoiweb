/**
 * 🏫 학교 탈출: SOS 보내기 — 마이크 대기시간·안내 회귀 하니스
 *
 * student-game-escape-voice.html의 형제 게임(student-game-escape-school.html). 그 파일이
 * 2026-07-31 실사용자 테스트로 고친 마이크/힌트/안내 버그를 이 새 게임도 전부 물려받았는지
 * 같은 방식으로 검증한다(파일·DOM id·STEPS 답만 다르고 엔진 코드는 사실상 동일 이식).
 *
 * 확인 항목
 ...
 *   ⑦ 7단계(교실→문→컴퓨터→쪽지→비밀번호→메시지→현관문)를 전부 말하면 탈출한다.
 *
 * 실행: node test-harness/escape_school_mic_harness.mjs
 */
...
const FILE = join(PUB, 'student-game-escape-school.html');
...
check('무엇을 말하면 되는지 예문이 설명에 들어 있다',
  /Look at the classroom/.test(introBlock));
check('마이크 안 될 때 타이핑으로도 된다고 알려준다',
  /키보드|타이핑|Typing/.test(introBlock));
check('7단계에 문이 열린다고 알려준다 (STEPS 길이=7과 일치해야 함)',
  /7번/.test(introBlock) && /Speak 7 times/.test(introBlock));
...
function boot() {
  ...
  vm.runInContext(code, win, { filename: 'student-game-escape-school.html' });
  return { win, els, click, txt, sr, clock };
}

/* ══ 3. 마이크 대기시간 — escape-voice.html에서 고친 결함을 이 게임도 상속했는가 ═══════════ */
console.log('\n🎤 마이크 대기시간 (버튼 누르고 말을 꺼내기까지)');
{
  ...
  g.sr.last.say('Look at the classroom');     // 이제서야 말을 꺼냈다
...
console.log('\n🙉 못 들었을 때');
{
  ...
  check('말할 문장을 함께 보여준다 (예전엔 "다시 말해보세요"로 끝났다)',
    /Look at the classroom/.test(g.txt('hint')), 'hint=' + g.txt('hint'));
}
...
console.log('\n💡 게임 중 안내');
{
  ...
  // 3단계(컴퓨터 켜기) 이후: 가만히 있으면 힌트가 한 칸씩 올라간다
  const g2 = boot();
  g2.click('btnStart');
  g2.win.G.i = 2; g2.win.showStep();          // index 2 = computer 단계
  check('3단계는 처음엔 정답을 안 보여준다 (스스로 생각할 시간)',
    !/Turn on the computer/.test(g2.txt('hint')), 'hint=' + g2.txt('hint'));
  g2.clock.tick(14500);
  ...
  check('끝내는 정답 문장까지 알려준다', /Turn on the computer/.test(g2.txt('hint')), 'hint=' + g2.txt('hint'));
...
console.log('\n🔁 틀렸을 때');
{
  const g = boot();
  g.click('btnStart');
  g.win.G.i = 2; g.win.showStep();            // computer 단계
  g.win.submitSaid('I do not know');
  check('오답 1회로도 힌트가 올라온다 (예전엔 3회 틀려야 나왔다)',
    /컴퓨터|turn|Turn/.test(g.txt('hint')), 'hint=' + g.txt('hint'));
  check('들린 말을 보여준다', /I do not know/.test(g.txt('heard')), 'heard=' + g.txt('heard'));
}

/* ══ 8. 끝까지 진행되는가 — 7단계 전부 말하면 탈출 ═══════════════════════ */
console.log('\n🚪 처음부터 끝까지 (교실 → 문 → 컴퓨터 → 쪽지 → 비밀번호 → 메시지 → 현관문)');
{
  const g = boot();
  g.click('btnStart');
  const answers = ['Look at the classroom','Open the door','Turn on the computer','Look at the note',
                   'three three five','Send the message','Open the door'];
...
console.log('\n⏭ 학교가 이야기하는 중에 마이크를 누르면');
{
  const g = boot();
  g.click('btnStart');
  g.click('micBtn'); g.sr.last.say('Look at the classroom');
...
/* ══ 10. 쪽지(noteCard) 팝업 — 포스트잇 단서를 실물처럼 보여주는가 ═══════════ */
console.log('\n📝 쪽지 팝업');
{
  const g = boot();
  g.click('btnStart');
  g.win.G.i = 3; g.win.showStep();            // note 단계
  g.win.submitSaid('Look at the note');
  check('쪽지 카드가 뜬다', g.els.noteCard._cls.has('show'));
  check('비밀번호 숫자가 쪽지에 적혀 있다', /three.*three.*five/i.test(g.txt('noteEn')), 'noteEn=' + g.txt('noteEn'));
}
