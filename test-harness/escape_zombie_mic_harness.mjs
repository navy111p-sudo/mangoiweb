/**
 * 🔬 좀비 실험실 탈출 — 마이크 대기시간·안내 회귀 하니스
 *
 * student-game-escape-zombie.html 은 student-game-escape-voice.html(2026-07-31 마이크 버그 수정판)의
 * 자매 게임으로, 같은 엔진·같은 검증된 페이싱을 그대로 물려받는다. 이 하니스는
 * test-harness/escape_voice_mic_harness.mjs 의 구조를 그대로 복사해 새 파일의 DOM id·STEPS 정답에
 * 맞춰 조정한 것이다 — 같은 결함(마이크 조기종료·힌트 속도·마이크-오디오 블리드 등)이
 * 새 파일에서 재발하지 않는지 지켜본다.
 *
 * 실행: node test-harness/escape_zombie_mic_harness.mjs
 */
...
const FILE = join(PUB, 'student-game-escape-zombie.html');
...
check('무엇을 말하면 되는지 예문이 설명에 들어 있다',
  /Look at the lab/.test(introBlock));
...
function boot() {
  ...
  vm.runInContext(code, win, { filename: 'student-game-escape-zombie.html' });
  return { win, els, click, txt, sr, clock };
}

/* ══ 3. 마이크 대기시간 — 이번 결함의 핵심(자매 게임과 동일하게 지켜야 함) ═══════ */
console.log('\n🎤 마이크 대기시간 (버튼 누르고 말을 꺼내기까지)');
{
  ...
  g.sr.last.say('Look at the lab');           // 이제서야 말을 꺼냈다
...
console.log('\n🙉 못 들었을 때');
{
  ...
  check('말할 문장을 함께 보여준다 (예전엔 "다시 말해보세요"로 끝났다)',
    /Look at the lab/.test(g.txt('hint')), 'hint=' + g.txt('hint'));
}
...
console.log('\n💡 게임 중 안내');
{
  ...
  // 3단계(컴퓨터 확인) 이후: 가만히 있으면 힌트가 한 칸씩 올라간다
  const g2 = boot();
  g2.click('btnStart');
  g2.win.G.i = 2; g2.win.showStep();
  check('3단계는 처음엔 정답을 안 보여준다 (스스로 생각할 시간)',
    !/Check the computer/.test(g2.txt('hint')), 'hint=' + g2.txt('hint'));
  g2.clock.tick(14500);
  ...
  check('끝내는 정답 문장까지 알려준다', /Check the computer/.test(g2.txt('hint')), 'hint=' + g2.txt('hint'));
...
console.log('\n🔁 틀렸을 때');
{
  ...
  check('오답 1회로도 힌트가 올라온다 (예전엔 3회 틀려야 나왔다)',
    /컴퓨터|check|Check/.test(g.txt('hint')), 'hint=' + g.txt('hint'));
  check('들린 말을 보여준다', /I do not know/.test(g.txt('heard')), 'heard=' + g.txt('heard'));
}

/* ══ 8. 쪽지·신음 연출 — 보관함을 연 직후 1회만 ══════════════════════════ */
console.log('\n📝 보관함 쪽지 · 신음 연출(게임당 1회)');
{
  const g = boot();
  g.click('btnStart');
  g.win.G.i = 1; g.win.showStep();             // cabinet 단계로 이동
  g.win.submitSaid('Open the cabinet');
  check('쪽지 카드가 뜬다', g.els.noteCard._cls.has('show'));
  check('쪽지 내용이 다음 단서(컴퓨터)를 가리킨다', /computer/i.test(g.txt('noteEn')), 'noteEn=' + g.txt('noteEn'));
  check('신음 연출 플래그가 1회만 켜진다', g.win.G.groaned === true, 'groaned=' + g.win.G.groaned);
}

/* ══ 9. 끝까지 진행되는가 — 7단계 전부 말하면 탈출 ═══════════════════════ */
console.log('\n🚪 처음부터 끝까지');
{
  const g = boot();
  g.click('btnStart');
  const answers = ['Look at the lab','Open the cabinet','Check the computer','Look at the logs',
                   'five one nine','Take the vaccine','Open the door'];
...
/* ══ 10. 내레이션 중 마이크 — 무반응으로 두지 않는다 ══════════════════════ */
console.log('\n⏭ 실험실이 이야기하는 중에 마이크를 누르면');
{
  const g = boot();
  g.click('btnStart');
  g.click('micBtn'); g.sr.last.say('Look at the lab');
