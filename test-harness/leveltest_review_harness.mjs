...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   [변이시험 — 2026-09-22, 15종 전부 실제 FAIL 확인]
     ...
     Ⓖ 해설 하나 비우기 2건 · Ⓗ questions 응답에 해설 싣기 1건 ·
   ...
   [🪤 이 검사가 처음에 «헛돌던» 것 셋 — 변이시험에서만 드러났다]
     ...
     ② 「그 호출이 try 안인가」를 «첫 번째 이름 위치» 로 물었더니 **import 줄이 잡혀**
        늘 참이었다. → try 블록을 중괄호 짝으로 잘라 «호출 위치가 그 범위 안인가» 로.
     ③ 중괄호 짝 자르기의 «꺾쇠 세기» 가 HTML 을 조립하는 JS 함수에서 엉뚱한 { 를
        잡아 멀쩡한 코드가 «r is not defined» 로 **거짓 실패**했다.
        → JS 선언에는 반환 타입이 없으니 «선언 뒤 첫 중괄호» 부터 짝만 맞춘다.
   ═══════════════════════════════════════════════════════════════════════ */
