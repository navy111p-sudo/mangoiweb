...
## approval-top-summary-browser.mjs — 결재 «맨 위 요약» (31건)
...
7. 언어 · 390px 에서 타일 값이 세로로 쪼개지지 않는지 · 경고 줄 WCAG 4.5.

> ⚠️ 요약 값을 **손으로 적지 않는다** — 정본 `foldHomeMoney` 를 돌려 그 결과를 스텁으로 쓴다.
>
> ⚠️ 변이 5종 실제 FAIL 확인 — 경고를 늘 그리면 3건, 범위 라벨을 빼면 2건,
> 「최근 N건만」을 지우면 1건, 「금액 없음」을 안 적으면 1건,
> 「진행 중」을 화면 15건으로 세면 1건.
