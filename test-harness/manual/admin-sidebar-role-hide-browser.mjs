...
(async () => {
  ...
  try {
    /* 이 화면의 href 항목 다섯 — 서버 게이트를 실측해 정한 기대값(2026-09-09).
       📌 **「수강 운영」은 2026-09-10 에 «감추는 쪽» 으로 바뀌었습니다.** 그날 사장님
          「수강 운영 게이트 달아줘」로 `/api/pay/enroll/admin/*` 가 본사 전용이
          됐기 때문입니다(`enrollAdminHqOnly`, src/enroll-ops.ts).
          ⚠️ 그전(09-09)에는 서버가 `checkAdminSession` 만 봐서 **일부러 안 감췄습니다** —
             서버가 여는 것을 화면만 감추면 진짜 구멍이 눈에서 사라지기 때문입니다.
             ⛔ 그 줄을 근거로 이 기대값을 «보인다» 로 되돌리지 마세요. 순서는
                «서버 먼저, 화면은 그 뒤» 이고 지금은 서버가 막고 있습니다. */
    const CASES = [
      { role: 'hq',       show: ['수업 관제탑', '수업 길이 변경', '수강 운영', '환불 처리', '영업 실적·평가'], hide: [] },
      { role: 'staff',    show: ['수업 관제탑', '수업 길이 변경', '수강 운영', '환불 처리', '영업 실적·평가'], hide: [] },
      { role: 'teacher',  show: ['수업 길이 변경'], hide: ['수업 관제탑', '수강 운영', '환불 처리', '영업 실적·평가'] },
      { role: 'branch',   show: ['수업 관제탑'],    hide: ['수업 길이 변경', '수강 운영', '환불 처리', '영업 실적·평가'] },
      { role: 'agency',   show: ['수업 관제탑'],    hide: ['수업 길이 변경', '수강 운영', '환불 처리', '영업 실적·평가'] },
      { role: 'franchise',show: ['수업 관제탑'],    hide: ['수업 길이 변경', '수강 운영', '환불 처리', '영업 실적·평가'] },
