// -*- coding: utf-8 -*-
// 👩‍🏫 A.i 친구하기 선생님 얼굴 크기 (웜업판: warmup-avatar-size-browser.mjs) — PC·폰 폭별로 «크게 · 안 찌그러짐 · 채팅을 안 가림 · 대화창이 남음» 을 잰다.
//   (2026-09-23 사장님 「선생님 얼굴들을 가장 적합한 위치에 지금보다 크게」)
//   자동으로 안 돕니다 — 사람이 부릅니다:
//     PW_DIR=/tmp/pw node test-harness/manual/ai-friend-avatar-size-browser.mjs
...
const SIZES=[[390,844,84,'폰 세로'],[360,640,68,'작은 폰'],[844,390,50,'폰 가로'],[768,1024,180,'태블릿'],
  [1024,768,120,'작은 노트북'],[1280,800,120,'노트북 1280'],[1366,768,120,'노트북 1366'],[1440,900,145,'PC 1440'],[1920,1080,180,'PC 1920']];
for(const [w,h,minW,name] of SIZES){
  ...
  await page.goto(base+'/ai-friend.html?x=1&_nc='+Date.now());
  await page.waitForTimeout(2500);
  const m=await page.evaluate(()=>{
    const R=e=>{const r=e.getBoundingClientRect();return {l:r.left,t:r.top,r:r.right,b:r.bottom,w:r.width,h:r.height};};
    const ring=document.getElementById('tavatar-ring'), log=document.querySelector('.chat'), wrap=document.querySelector('.chat');
  ...
  ok(name+': 얼굴이 대화창을 안 가림(대화창 위에 놓임)', m.rr.b<=m.wr.t+1, `얼굴 아래 ${Math.round(m.rr.b)} · 대화창 위 ${Math.round(m.wr.t)}`);
  const wraps=await page.evaluate(()=>{const n=e=>{if(!e)return 0;const lh=parseFloat(getComputedStyle(e).lineHeight)||parseFloat(getComputedStyle(e).fontSize)*1.4;return Math.round(e.getBoundingClientRect().height/lh);};return {st:n(document.querySelector('#tavatar-status .ts-idle')),pg:n(document.querySelector('.top .name'))};});
  ok(name+': 「함께 말해 봐요」·제목이 한 줄', wraps.st<=1 && wraps.pg<=1, JSON.stringify(wraps));
  ok(name+': 대화창이 충분히 남음(≥ 화면의 25%)', m.lr.h>=m.innerH*0.25, Math.round(m.lr.h)+' / '+m.innerH);
  await page.screenshot({path:`/tmp/claude-0/-home-user-mangoiweb/11708116-d8f5-581d-b17e-b8e1f5a4c520/scratchpad/af-${w}x${h}.png`});
  await ctx.close();
}
