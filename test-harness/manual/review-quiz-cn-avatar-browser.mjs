// -*- coding: utf-8 -*-
// 👩‍🏫 중국어 복습퀴즈(review-quiz-cn) 메이 선생님 — 2026-09-24 사장님 «중국어 복습퀴즈 화면에도 메이 선생님».
//   재는 것: ① 폭별 크기·4:5·화면 안·안 가려짐·퀴즈(zoom 걸린 .wrap)/메뉴 손잡이와 안 겹침
//            ② 메이 얼굴(mei-closed.webp)이 실제로 그려지는가
//            ③ 듣기 문항을 읽어 주면 «말하는 중» 이 되고 끝나면 풀리는가 — 두 번째 재생도,
//               그리고 잠금 해제용 «무음» 재생에는 입이 안 움직이는가(짝)
//   자동으로 안 돕니다 — 사람이 부릅니다:  PW_DIR=/tmp/pw node test-harness/manual/review-quiz-cn-avatar-browser.mjs
...
const SIZES=[[390,844,105,'폰 세로'],[360,640,86,'작은 폰'],[844,390,56,'폰 가로'],[768,1024,145,'태블릿'],
  [1024,768,125,'작은 노트북'],[1280,800,135,'노트북 1280'],[1440,900,135,'PC 1440'],[1600,900,160,'PC 1600'],[1920,1080,290,'PC 1920']];
const QUIZ={id:1,title:'第一课',questions:[{idx:0,type:'listen',q:'들은 것을 고르세요',opts:['你好','谢谢','再见','对不起']},{idx:1,type:'choice',q:'你好?',opts:['안녕','고마워','잘가','미안']}]};
async function open(w,h){
  const ctx=await browser.newContext({viewport:{width:w,height:h}});
  await ctx.addInitScript(()=>{try{localStorage.setItem('mangoi_logged_user',JSON.stringify({uid:'tester',name:'테스터',role:'student'}));}catch(e){}});
  const page=await ctx.newPage();
  await page.route('**/*',async route=>{const u=new URL(route.request().url());if(u.origin!==base){await route.abort();return;}
    if(u.pathname==='/api/review-quiz/tts'){await route.fulfill({status:200,headers:{'content-type':'audio/wav'},body:WAV});return;}
    if(u.pathname==='/api/review-quiz/list'){await route.fulfill({json:{ok:true,quizzes:[{id:1,title:'第一课',question_count:2}]}});return;}
    if(u.pathname==='/api/review-quiz/get'){await route.fulfill({json:{ok:true,quiz:QUIZ}});return;}
    if(u.pathname.startsWith('/api/')){await route.fulfill({json:{ok:true}});return;}
    await route.continue();});
  await page.goto(base+'/review-quiz-cn.html?_nc='+Date.now());
...
for(const [w,h,minW,name] of SIZES){
  ...
  const m=await page.evaluate(()=>{
    ...
    const rr=R(ring), q=R(document.querySelector('.wrap')), intro=R(document.querySelector('.intro'));
    ...
    return {rr,q,intro,fixed,tr,ov,onTop:!!(top&&ring.contains(top)),stLines:Math.round(st.getBoundingClientRect().height/lh),innerH:innerHeight,innerW:innerWidth};
  });
  console.log(`▶ ${name} ${w}x${h} — 얼굴 ${Math.round(m.rr.w)}x${Math.round(m.rr.h)} · ${m.fixed?'오른쪽 여백(고정)':'퀴즈 위'} · 소개 상자 위 ${Math.round(m.intro.t)}`);
  ...
  ok(name+': 가로 넘침 없음', !m.ov);
  if(m.fixed) ok(name+': 퀴즈(zoom 포함)와 안 겹침', m.rr.l>=m.intro.r+8, `얼굴 왼쪽 ${Math.round(m.rr.l)} · 퀴즈 오른쪽 ${Math.round(m.intro.r)}`);
  else { ok(name+': 퀴즈 위에 놓임(안 가림)', m.rr.b<=m.intro.t+1, `얼굴 아래 ${Math.round(m.rr.b)} · 퀴즈 위 ${Math.round(m.intro.t)}`);
         ok(name+': 퀴즈 소개가 첫 화면 안에서 시작', m.intro.t < m.innerH*0.7, Math.round(m.intro.t)+' / '+m.innerH); }
  if(m.tr) ok(name+': 메뉴 손잡이와 안 겹침', m.rr.l>=m.tr.r || m.rr.b<=m.tr.t || m.rr.t>=m.tr.b);
  ok(name+': 상태 글자 한 줄', m.stLines<=1, String(m.stLines));
  await page.screenshot({path:`/tmp/claude-0/-home-user-mangoiweb/11708116-d8f5-581d-b17e-b8e1f5a4c520/scratchpad/rq-${w}x${h}.png`});
  await ctx.close();
}
{
  reqs.length=0;
  ...
  // «끝 상태» 만 보면 무음이 금방 끝나 늘 통과한다 — 한 번이라도 켜졌는지 기록한다.
  await page.evaluate(()=>{window.__spEver=false;const w=document.getElementById('tavatar-wrap');
    new MutationObserver(()=>{if(w.classList.contains('speaking'))window.__spEver=true;}).observe(w,{attributes:true,attributeFilter:['class']});});
  await page.mouse.click(5,300); await page.waitForTimeout(400);
  ok('잠금 해제용 무음 재생에는 입이 한 번도 안 움직임(짝)', !(await page.evaluate(()=>window.__spEver)));
  await page.click('.quiz-item'); await page.waitForTimeout(800);
  ok('듣기 문항 자동 재생: 말하는 중', await sp());
  await page.waitForTimeout(1800);
  ok('다 읽으면 말하는 중 풀림', !(await sp()));
  await page.click('#playBtn'); await page.waitForTimeout(500);
  ok('「다시 듣기」도 말하는 중(오디오 요소 재사용)', await sp());
  await page.waitForTimeout(1800);
  ok('두 번째도 끝나면 풀림', !(await sp()));
  await ctx.close();
}
