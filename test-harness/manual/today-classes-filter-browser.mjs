...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   (2026-09-01 신설 · 2026-09-08 ⑨절 「23시 이후」 · 2026-09-09 ⑩절 「야간」 추가)
...
const BOOT = `
  ...
  (function(){
    ...
    /* 🌙 ⑨⑩절(시간대 거르기)에서 씨앗을 바꿔 끼운다 — 위 ①~⑧ 의 기대값을 건드리지 않으려고
...
async function main() {
  ...
  const LATE_SEED = [
    ...
    { source: 'cafe24', schedule_id: null, observable: false, room_id: 'c24-e', student_uid: 'e',
      student_name: '다섯시오십구', teacher_name: 'T', level: null, textbook: null, textbook_assigned: false,
      start_time: '05:59', start_ts: kst(5, 59), end_ts: kst(6, 19), status: 'early', join_open: false, is_level_test: false },
    { source: 'cafe24', schedule_id: null, observable: false, room_id: 'c24-f', student_uid: 'f',
      student_name: '여섯시정각', teacher_name: 'T', level: null, textbook: null, textbook_assigned: false,
      start_time: '06:00', start_ts: kst(6, 0), end_ts: kst(6, 20), status: 'early', join_open: false, is_level_test: false },
  ...
  /* ⚠️ `new Event('change')` 는 bubble 하지 않는다 — {bubbles:true} 를 빼면 위임 리스너가
     안 깨어나 «코드가 틀린 것처럼» 보인다(CLAUDE.md 2장). */
  const setNight = async v => ev(`(function(){var c=document.getElementById("tc-night");c.value=${JSON.stringify(v)};c.dispatchEvent(new Event("change",{bubbles:true}));})()`);
  check('선택칸(#tc-night)이 있다', await ev('!!document.getElementById("tc-night")'));
  check('   화면에 실제로 보인다 (카드가 열려 있고 무엇에도 안 덮인다)',
    await ev('!!document.getElementById("tc-night").offsetParent'));
  check('   #tc-body «밖» 이다 (표를 다시 그려도 고른 값이 안 풀린다)',
    await ev('!document.getElementById("tc-body").contains(document.getElementById("tc-night"))'));
  check('   갈래가 셋이다 (전체 · 23시 이후 · 야간)',
    (await ev('[...document.querySelectorAll("#tc-night option")].map(o=>o.value).join(",")')) === ',late,night');
  check('전체를 고르면 여섯 건 다 보인다', (await ev('document.querySelectorAll("#tc-body tbody tr").length')) === 6);
  await setNight('late'); await sleep(150);
  ...
  check('   «표시 2건 / 전체 6건» 처럼 둘 다 말한다', /표시 2건/.test(cntL) && /전체 6건/.test(cntL), cntL);
  const hitsL = await ev('window.__tcHits');
  await setNight(''); await setNight('late'); await sleep(150);
  ...
  check('   전체로 되돌리면 그 줄이 돌아온다 (거르개가 줄을 영영 삼키지 않는다)',
    (await ev('document.querySelectorAll("#tc-body tbody tr").length')) === 1);

  console.log('\n── ⑩ 🌒 「야간 — 자정 이후 포함」 (2026-09-09) ────────');
  await ev('window.__tcSetSessions(' + JSON.stringify(LATE_SEED) + ')');
  await ev('tcLoadToday()'); await sleep(600);
  await setNight('night'); await sleep(150);
  check('자정 00:10 이 «들어온다» (23시 이후가 빠뜨리던 그 줄)',
    /자정십분/.test(await ev('document.getElementById("tc-body").textContent')), await names());
  check('   새벽 끝 05:59 도 들어온다', /다섯시오십구/.test(await ev('document.getElementById("tc-body").textContent')));
  check('   23:00·23:59 도 함께 남는다 (야간 = 밤 + 새벽)',
    /스물세시정각/.test(await ev('document.getElementById("tc-body").textContent'))
    && /스물세시오십구/.test(await ev('document.getElementById("tc-body").textContent')));
  check('   06:00·22:59 는 빠진다 (거기부터는 아침·저녁)',
    !/여섯시정각|스물두시오십구/.test(await ev('document.getElementById("tc-body").textContent')), await names());
  check('   남은 줄이 네 개다', (await ev('document.querySelectorAll("#tc-body tbody tr").length')) === 4);
  const cntN = await ev('document.getElementById("tc-count").textContent');
  check('   «표시 4건 / 전체 6건» 처럼 둘 다 말한다', /표시 4건/.test(cntN) && /전체 6건/.test(cntN), cntN);
  /* 🌒 «자정을 넘겨 이어지는 한 판» 은 두 날짜로 갈린다 — 화면이 그 사실을 말해야 한다 */
  const bodyN = await ev('document.getElementById("tc-body").textContent');
  /* ⛔ 건수를 손으로 적지 말 것 — 씨앗을 고치면 조용히 어긋난다(CLAUDE.md 2장 「설정표를 손으로 적으면」).
     KST 시(hour)로 «씨앗에서 세어» 기대값을 만든다. */
  const seedH = LATE_SEED.map(r => new Date(r.start_ts + 9 * 3600 * 1000).getUTCHours());
  const seedDawn = seedH.filter(h => h >= 0 && h < 6).length;
  const seedLate = seedH.filter(h => h >= 23).length;
  check(`날짜 경계를 화면이 말한다 (새벽 ${seedDawn}건 + 밤 ${seedLate}건 · 이어지는 새벽은 다음 날짜 목록)`,
    new RegExp('새벽 ' + seedDawn + '건').test(bodyN) && new RegExp('밤 ' + seedLate + '건').test(bodyN)
    && /다음 날짜/.test(bodyN), bodyN.slice(0, 200));
  await setNight('late'); await sleep(150);
  check('   「23시 이후」로 바꾸면 그 안내 줄이 사라진다 (뜻 없는 문장을 늘 띄우지 않는다)',
    !/다음 날짜/.test(await ev('document.getElementById("tc-body").textContent')));
  const hitsN = await ev('window.__tcHits');
  await setNight('night'); await setNight(''); await setNight('night'); await sleep(150);
  check('   화면 안에서만 거른다 (서버 요청 0건)', (await ev('window.__tcHits')) === hitsN);
  await setNight(''); await sleep(150);
  ...
  await ev('(function(){ window.adminLang="en"; applyAdminLangDom(); document.dispatchEvent(new CustomEvent("mangoi:lang-changed")); })()');
  await sleep(250);
  const lateEn = await ev('[...document.querySelectorAll("#tc-night option")].map(o=>o.textContent.trim()).join("|")');
  check('EN 으로 바꾸면 세 갈래 라벨이 모두 영어다',
    /All/.test(lateEn) && /After 23:00/i.test(lateEn) && /Night/i.test(lateEn), lateEn);
