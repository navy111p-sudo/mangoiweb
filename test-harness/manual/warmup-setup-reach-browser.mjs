...
//     Ⓔ .menu-scroll 의 overflow-y:auto 제거 → 실패 2 (카드가 다시 화면 밖으로)
//     되돌린 뒤 기준 점수 = 통과 72 · 실패 0
//   🔴 [안 잡히는 변이 — 알고 두는 구멍]  min-height:0 «만» 지운 판은 72/0 으로 통과한다.
//      flex 자식의 자동 최소 크기는 계산된 overflow 가 visible 일 때만 걸리므로(CSS Flexbox 4.5)
//      overflow-y:auto 가 있는 동안 그 줄은 겹쳐 있는 보호다. 지우지는 말되(overflow 를 되돌리는
//      날 일한다) «그 줄이 지금 카드를 붙잡고 있다» 고 적지도 말 것 — 제가 그렇게 적었다가
//      함정 대조 실측으로 정정했다.
