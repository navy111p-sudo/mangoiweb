// -*- coding: utf-8 -*-
// 🀄 중국어 웜업(메이·룽) 선생님 얼굴 크기 — 영어보다 한 단계 크게(html.wu-zh).
//   (2026-09-24 사장님 「중국어 웜업 화면에도 메이 선생님 크게」)
//   자동으로 안 돕니다 — 사람이 부릅니다:
//     PW_DIR=/tmp/pw node test-harness/manual/warmup-zh-avatar-size-browser.mjs
//   ⚠️ 영어 쪽은 warmup-avatar-size-browser.mjs 가 옛 크기 그대로인지 봅니다(짝).
...
const SIZES=[[390,844,120,'폰 세로'],[360,640,100,'작은 폰'],[844,390,60,'폰 가로'],[768,1024,146,'태블릿'],
...
for(const [w,h,minW,name] of SIZES){
  ...
  await page.goto(base+'/warmup.html?setup=0&lang=zh&_nc='+Date.now());
  ...
  ok(name+': 중국어 표시(html.wu-zh)가 붙음', await page.evaluate(()=>document.documentElement.classList.contains('wu-zh')));
  ...
  ok(name+': 「함께 말해 봐요」 한 줄 · 제목은 두 줄까지', wraps.st<=1 && wraps.pg<=(w<=400?2:1), JSON.stringify(wraps));
  const ov=await page.evaluate(()=>{const p=document.querySelector('.wg-progress');const r=p?p.getBoundingClientRect():{right:0};return {pr:Math.round(r.right),sw:document.documentElement.scrollWidth,iw:innerWidth};});ok(name+': 제목이 화면 밖으로 안 넘침',ov.pr<=ov.iw+1&&ov.sw<=ov.iw,JSON.stringify(ov));
  ok(name+': 작업칸(그림·대화)이 충분히 남음(≥ 화면의 25%)', m.lr.h>=m.innerH*0.25, Math.round(m.lr.h)+' / '+m.innerH);
  await page.screenshot({path:`/tmp/claude-0/-home-user-mangoiweb/11708116-d8f5-581d-b17e-b8e1f5a4c520/scratchpad/zh-av-${w}x${h}.png`});
  await ctx.close();
}
