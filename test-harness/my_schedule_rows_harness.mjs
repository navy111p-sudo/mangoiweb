...
console.log('▶ my_schedule_rows_harness — 홈 «내 수업» 카드 세로 목록');
{
  ...
  ok(/nms-next/.test(rows[0] || '') && /다음 · 9\/15\(화\)/.test(rows[0] || ''), '① 다음 수업이 맨 위 + 「다음 · 9/15」', rows[0]);
...
}
{
  ...
  const rows = rowsOf(r.html);
  ok(rows.length === 3 && txt(rows[1], 'nms-day') === '9/20(일)' && (rows[1].match(/9\/20/g) || []).length === 1, '④ 일회성은 날짜(+요일)가 요일 칸에 한 번만', rows[1]);
  ok(txt(rows[2], 'nms-teacher') === 'C' && !/nms-date/.test(rows[2]), '④ 다음 날짜를 못 구한 줄은 맨 아래, 날짜 칸 없음');
  const r1 = await render([S(9, { ko: [], en: [] }, '14:00', 'MAIMAI', '2026-09-18', 1, { scheduled_date: '2026-09-18' })]);
  const row1 = rowsOf(r1.html)[0] || '';
  ok(/nms-next/.test(row1) && txt(row1, 'nms-day') === '9/18(금)' && txt(row1, 'nms-date') === '다음', '④ 일회성이 «다음» 이면 요일 칸 「9/18(금)」 + 태그 「다음」만', row1);
}
{
  // ⑤ 영어
  const r = await render([S(1, D.화, '19:20', 'A', '2026-09-15', 1), S(2, D.목, '19:20', 'A', '2026-09-17', 3)], 'en');
  const rows = rowsOf(r.html);
  ok(txt(rows[0], 'nms-day') === 'Tue/Thu' && /7:20 PM/.test(rows[0]) && /Next · 9\/15\(Tue\)/.test(rows[0]) && /My classes/.test(r.html), '⑤ EN: Tue/Thu · 7:20 PM · Next · 9/15', rows[0]);
}
{
...
{
  // ⑦ 계약: idx-cta-status.js 의 classSub() 가 이 카드에서 «다음 수업» 줄을 읽는다 — 그 함수를 오려 내 실제로 돌린다
  const cta = fs.readFileSync(path.join(ROOT, 'cloudflare-deploy/public/js/idx-cta-status.js'), 'utf8');
  const m = /function classSub\(\) \{([\s\S]*?)\n  \}/.exec(cta);
  ok(!!m, '⑦ 전제: idx-cta-status.js 에서 classSub() 몸통을 오려 냈다');
  const r = await render([S(1, D.화, '19:20', '강선생님', '2026-09-15', 1), S(2, D.수, '17:00', 'Kaye', '2026-09-16', 2)]);
  const firstOf = (html, cls) => { const re = new RegExp('<span class="([^"]*\\b' + cls + '\\b[^"]*)"[^>]*>([^<]*)<'); const x = re.exec(html); return x ? { className: x[1], textContent: x[2] } : null; };
