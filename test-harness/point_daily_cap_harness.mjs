/* ═══════════════════════════════════════════════════════════════════════════
   ...
   🔴 [이 상한은 아직 «전체» 가 아니다 — 알고 쓸 것]
     ...
     🔀 (2026-09-01 갱신 — PR #670 이 병합되며 «사실» 이 바뀌었다)
       · api-games.ts 단어장·복습퀴즈 보상 — **원장으로 합쳐졌고**(잔액 직접올림 0곳),
         **2026-09-01 사장님 결정으로 총량 100점도 지난다**(`dailyAllowance`).
         그전 실효 상한은 단어장 400 + 미션 50 + 복습퀴즈 500 + 총량 100 = **하루 1,050점**이었다.
...
console.log('\n[ D. 이 상한이 «전체» 가 아니라는 것을 알고 있다 ]');
{
  ...
  /* 🔴 (2026-09-01 뒤집음) 원래 이 자리는 「총량 상한이 그 둘을 **세지 않는다**」였다.
     사장님 결정으로 100점 통일이 됐으므로 반대를 지킨다.
     ⚠️ 그때 검사는 «파일에 그 이름이 있는가» 였는데, 목록을 비운 뒤에도 **내가 쓴 주석에
        남은 이름을 잡아 24/24 로 통과했다.** 문자열이 아니라 **선언된 값**을 본다. */
  check('CAP_UNCOUNTED_RULES 가 비어 있다(상한 100점 통일)',
    /export const CAP_UNCOUNTED_RULES[^=]*=\s*\[\s*\]/.test(policy),
    '선언부: ' + (policy.match(/export const CAP_UNCOUNTED_RULES[^;]*/) || ['(못 찾음)'])[0].slice(0, 90)
    + '\n       → 여기에 규칙이 있으면 그 경로만 상한 밖으로 빠져나간다');
  check('  · 적립하는 쪽도 총량 예산을 지난다',
    /dailyAllowance\(env as any, uid, 'vocab_review'\)/.test(games)
      && /dailyAllowance\(env as any, userId, 'review_quiz_done'\)/.test(games),
    '목록만 비우면 «남의 예산만 깎아먹고 자기는 안 막히는» 최악이 된다');
