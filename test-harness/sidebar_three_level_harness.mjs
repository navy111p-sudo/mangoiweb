...
check(`그 항목들이 전부 자기 설명을 갖는다${noTip.length ? ' — 없는 것: ' + noTip.map((x) => x.ko).join(', ') : ''}`,
  noTip.length === 0);
// 📚 (2026-09-25) 「수강 운영」이 강사 그룹 → 학생 그룹으로 이사했다. 옛 이름·옛 자리 둘 다
//    «지금 실재하는 자리» 로 이어져야 한다 — 목적지는 글자로 못 박지 않고 GROUPS 에서 찾는다.
check('「수강 운영」 이름을 바꿨으니 이사표에 한 줄 적혀 있다 — 없으면 마지막 화면이 「오늘의 수업」으로 튄다',
  /'teacher:수강 운영\(배율·정원\)':\s*'(\w+):수강 운영'/.test(ia6)
  && /'teacher:수강 운영':\s*'student:수강 운영'/.test(ia6));
