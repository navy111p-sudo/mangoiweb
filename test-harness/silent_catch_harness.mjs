...
//
// ⚠️ 알려진 한계 — «읽기» 실패도 똑같이 위험할 수 있다. 이 하니스는 그걸 못 잡는다.
//   실제 사례(2026-08-09, lesson-reminder.ts):
//       try { const dup = await …SELECT 1 FROM lesson_reminder_log…; if (dup) continue; } catch {}
//   이 «중복 확인» 조회가 실패하면 코드는 「아직 안 보냄」으로 간주하고 그대로 문자를 보낸다.
//   → 학부모 휴대폰에 같은 안내가 두 번 간다. 쓰기는 한 줄도 없는데 피해는 바깥으로 나간다.
//   패턴을 일반적으로 잡기는 어렵다(「무엇을 확인하는 읽기인지」를 알아야 한다).
//   그러니 **중복·멱등 확인용 SELECT 를 조용히 삼키지 말 것** — 사람이 지켜야 하는 규칙이다.
//
