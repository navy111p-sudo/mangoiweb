...
{
  /* ⚠️ (2026-08-06 정책 변경) 예전엔 '입장 시 배정 교재를 자동으로 띄우는 경로'를 지켰다.
     그런데 그 자동 로드가 «강사가 보던 교재를 덮어쓰는» 원인이었다(사장님 신고:
     "나갔다 들어오니 교재가 다른 게 보여"). 강사 계정은 배정이 없어 항상
     'Mangoi Books' 로 폴백해 공유까지 했다.
     새 규칙: 수업 화면 교재는 «강사가 올린 것»만. 없으면 대기 카드.
     여기서는 그 규칙이 뒤집히지 않았는지만 지킨다(상세 검증은 vc_textbook_entry_harness). */
  check('입장 시 배정 교재를 자동으로 띄우지 않는다 (강사 교재 덮어쓰기 방지)',
        !/vcAutoLoadStudentTextbook\(\)\s*\.then/.test(indexHtml),
        '자동 로드가 살아나면 재입장마다 다른 교재가 뜬다');
  check('특정 교재를 코드에 하드코딩해 강제하지 않는다',
        !/vcApplySharedPdf\([^)]*['"](MES|BTS)/.test(indexHtml),
        '교재 배정은 데이터 문제 — 코드로 박으면 영영 안 고쳐진다');
}
