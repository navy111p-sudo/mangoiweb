...
check('교재 업로더 화면이 있다', UP.length > 0);
{
  ...
  if (rxSrc && fnSrc) {
    ...
    /* 📌 (2026-08-27 정정) 이 줄은 여전히 «쪼개진다» 를 못 박지만, 쪼개진 «이름» 이 달라졌다.
       그전에는 교재명이 **「001」** 이 되어 책 이름이 통째로 사라졌고(다른 책의 001 과
       공용 자료실에서 한 묶음으로 합쳐졌다), 지금은 「다락원 중국어 마스터 3 001」 이다.
       즉 이름은 살아남지만 **여전히 유닛마다 다른 권**이라 중국어 매칭은 그대로 안 맞는다
       — 다락원 재업로드에 숫자 폴더를 쓰면 안 되는 이유는 바뀌지 않았다.
       자세한 감시: test-harness/textbook_uploader_naming_harness.mjs */
    check('숫자 폴더(001)는 «유닛마다 다른 책» 으로 쪼개진다 — 다락원에는 쓰지 말 것 (실측 «' + bad2.textbook + '»)',
      bad2.textbook !== BOOK);
    check('그래도 책 이름은 남는다 — 「001」 처럼 번호만 되지는 않는다',
      /^다락원 중국어 마스터 3/.test(bad2.textbook));
  }
}
